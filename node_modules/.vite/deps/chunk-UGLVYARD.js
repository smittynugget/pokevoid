...
var Pen = class {
  ...
  resetFromJSON(o) {
    this.text = GetValue2(o, "text", "");
    this.x = GetValue2(o, "x", 0);
    this.y = GetValue2(o, "y", 0);
    this.width = GetValue2(o, "width", 0);
    var prop = GetValue2(o, "prop", null);
    if (prop === null) {
      prop = {};
    }
    this.prop = prop;
    this.newLineMode = GetValue2(o, "newLineMode", 0);
    this.startIndex = GetValue2(o, "startIndex", 0);
  }
  get plainText() {
    var txt = this.text;
...
var CanvasText = class {
  constructor(config) {
    ...
    this.context = GetValue3(config, "context", null);
    this.canvas = this.context.canvas;
    this.parser = GetValue3(config, "parser", null);
    this.defaultStyle = GetValue3(config, "style", null);
    this.autoRound = true;
    this.pensPool = config.pensPool;
    this.linesPool = config.linesPool;
...
var AddImage = function(key, config) {
  ...
  this.images[key] = {
    ...
    left: GetValue4(config, "left", 0),
    right: GetValue4(config, "right", 0),
    originX: GetValue4(config, "originX", 0),
    originY: GetValue4(config, "originY", 0),
    tintFill: GetValue4(config, "tintFill", false)
  };
};
...
var Text = class extends TextBase_default {
  constructor(scene, x, y, text, style, type, parser) {
    if (IsPlainObject2(x)) {
      var config = x;
      x = GetValue5(config, "x", 0);
      y = GetValue5(config, "y", 0);
      text = GetValue5(config, "text", "");
      style = GetValue5(config, "style");
    }
    if (x === void 0) {
      x = 0;
    }
    ...
    var imageData = GetValue5(style, "images", void 0);
    if (imageData) {
    ...
    var sharedPoolMode = GetValue5(style, "sharedPool", true);
    ...
    this.setText(text);
    this.setUrlTagCursorStyle(GetValue5(style, "urlTagCursorStyle", "pointer"));
    if (GetValue5(style, "interactive", false)) {
      this.setInteractive();
    }
  }
  preDestroy() {
  ...
  setPadding(left, top, right, bottom) {
    if (typeof left === "object") {
      var config = left;
      var x = GetValue5(config, "x", null);
      if (x !== null) {
        left = x;
        right = x;
      } else {
        left = GetValue5(config, "left", 0);
        right = GetValue5(config, "right", left);
      }
      var y = GetValue5(config, "y", null);
      if (y !== null) {
        top = y;
        bottom = y;
      } else {
        top = GetValue5(config, "top", 0);
        bottom = GetValue5(config, "bottom", top);
      }
    } else {
      if (left === void 0) {
        left = 0;
...
var Parser = class {
  constructor(style) {
    var delimiters = GetValue6(style, "delimiters", "[]");
...
phaser3-rex-plugins/plugins/gameobjects/textbase/textstyle/MeasureText.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-UGLVYARD.js.map
