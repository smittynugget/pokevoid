...
export default class VouchersUiHandler extends MessageUiHandler {
  ...
  protected showVoucher(voucher: Voucher) {
    ...
    const unlocked = voucher && voucher.id && voucherUnlocks.hasOwnProperty(voucher.id);
    this.titleText.setText(voucher ? getVoucherTypeName(voucher.voucherType) : '');
    this.showText(voucher ? voucher.description : '');
  ...
  setCursor(cursor: integer): boolean {
    ...
    const selectedVoucherKey = voucherKeys[cursor + this.scrollCursor * itemCols];
    const selectedVoucher = vouchers[selectedVoucherKey];
    this.showVoucher(selectedVoucher);
    }
