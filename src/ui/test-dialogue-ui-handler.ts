...
export default class TestDialogueUiHandler extends FormModalUiHandler {
  ...
  setup() {
    ...
    const keysInArrays = flattenKeys(i18next.getDataByLanguage(String(i18next.resolvedLanguage))).filter((t) => t.length > 0);
    const keys = keysInArrays.flat(Infinity).map(String);
    this.keys = keys;
  }
