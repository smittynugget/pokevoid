...
export default class ChampionSelectUiHandler extends ModalUiHandler {
  ...
  private getCurrentForSegment(segment: { types: Type[]; amount: number }): number {
    ...
    const isUnlocked = this.championManager.isChampionUnlocked(selectedChampionId) ||
                     (this.championManager.getChampionData(selectedChampionId)?.isUnlocked === true);
  ...
  processInput(button: Button): boolean {
    ...
    const isUnlocked = this.championManager.isChampionUnlocked(selectedChampionId) || champ?.isUnlocked === true;
