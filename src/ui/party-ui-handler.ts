...
export default class PartyUiHandler extends MessageUiHandler {
    ...
    if (this.optionsMode) {
      ...
      if (button === Button.ACTION) {
        ...
        if (this.partyUiMode === PartyUiMode.MODIFIER_TRANSFER && !this.transferMode && option !== PartyOption.CANCEL) {
          ...
          if (option !== PartyOption.TRANSFER && option !== PartyOption.SPLICE && option !== PartyOption.SACRIFICE) {
          ...
          this.clearOptions();
          ui.playSelect();
          if (this.cursor >= this.scene.currentBattle.getBattlerCount() || !pokemon.isAllowedInBattle() || this.partyUiMode == PartyUiMode.ADDPOKEMON) {
    ...
    if (this.partyUiMode !== PartyUiMode.MOVE_MODIFIER && this.partyUiMode !== PartyUiMode.REMEMBER_MOVE_MODIFIER && (this.transferMode || this.partyUiMode !== PartyUiMode.MODIFIER_TRANSFER)) {
      ...
      case PartyUiMode.SACRIFICE:
        if (this.transferMode) {
          ...
          if (this.cursor !== this.transferCursor && (this.cursor !== 0 || this.hasEnoughCollectedTypeModifiers(pokemon))) {
  ...
  getPokemonTradeValue(pokemon: PlayerPokemon): integer {
    ...
    const coinRelicValue = this.scene.getWaveMoneyAmount(10); // Coin Relic value
