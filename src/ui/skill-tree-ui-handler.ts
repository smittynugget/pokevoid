...
export default class SkillTreeUiHandler extends ModalUiHandler {
  ...
  private getVisibleNodes(): SkillTreeNode[] {
    ...
    const maxDepth = this.config.activeSkillTree.maxVisibleDepth;
  ...
  private generateSkillTree(): void {
    ...
    if (cached && cached.length && (isSelection || hasRoot || isEnhancedDebug)) {
      if (isSelection) {
        const maxDepth = this.debugDepthOverride > 0 ? this.debugDepthOverride : 2;
    ...
    this.scene.executeWithSeedOffset(() => {
      ...
      treeNodes = gen.generateCompleteTree(maxDepth);
    ...
    switch (node.rewardData?.type) {
      ...
      case SkillTreeRewardType.MEGA_STONE:
        ...
        if (megaStone && typeof megaStone === "number") {
          const megaStoneName = FormChangeItem[megaStone]?.toLowerCase();
          if (megaStoneName) {
  ...
  private restoreNormalInstructions(): void {
    ...
    try {
      if (this.config.mode === SkillTreeMode.POKEMON_SELECTION) {
        ...
        const key = hasSig && !hasGen
          ? "skillTree:instructionsSelectGeneral"
          : (!hasSig && hasGen ? "skillTree:instructionsSelectSignature" : "skillTree:instructionsSelectBoth");
        this.instructionsText.setText(i18next.t(key));
      } else {
        const instructionsKey = this.getInstructionsKey();
  ...
  private updateInstructionBackground(): void {
    ...
    this.instructionsTextBg.fillStyle(c.BG_COLOR, c.BG_ALPHA);
  ...
  private playUnlockEffect(node: SkillTreeNode): void {
    ...
    this.scene.time.delayedCall(c.DELAY, () => {
      ...
      try {
        (this.scene as BattleScene).playSound("battle_anims/PRSFX- Last Resort1");
      } catch {}
    });
  }
  ...
  private handleSelection(node: SkillTreeNode): boolean {
    ...
    if (this.config.mode === SkillTreeMode.POKEMON_SELECTION) {
      ...
      const existingOfType = this.nodes.filter(n =>
        this.config.activeSkillTree.unlockedNodes.has(n.id) &&
        n.rewardData.type === (isSignature ? SkillTreeRewardType.SIGNATURE_POKEMON : SkillTreeRewardType.GENERAL_POKEMON)
      );

      if (existingOfType.length >= 1) {

        this.showSelectionLimitMessage(isSignature ? "signature" : "general");
        return false;
      }
    }
  ...
  private handlePurchase(node: SkillTreeNode): boolean {
    if (!this.canPurchase(node)) {
      const nodeCost = this.getNodeCost(node);
      (this.scene as BattleScene).ui.showText(i18next.t("skillTree:cannotAfford", { cost: nodeCost, current: this.config?.activeSkillTree.skillPoints || 0, defaultValue: "Cannot purchase" }), null, () => {}, null, true);
  ...
  private applyRewardToGameState(node: SkillTreeNode): void {
    if (!this.config) {
      console.warn(`[SKILL_TREE_DIAG] applyRewardToGameState ABORTED: config is null/undefined`);
      return;
    }
    const ast = this.config.activeSkillTree;
    switch (node.rewardData?.type) {
