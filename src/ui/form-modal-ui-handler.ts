...
export abstract class FormModalUiHandler extends ModalUiHandler {
  ...
  getReadableErrorMessage(error: string): string {
    ...
    protected getInputConfig(field: string): InputText.IConfig {
        return {
            type: field.includes(i18next.t("menu:password")) || field.includes(i18next.t("menu:confirmPassword"))
                ? "password"
                : "text",
            maxLength: field.includes(i18next.t("menu:password")) || field.includes(i18next.t("menu:confirmPassword"))
                ? 64
                : 18
        };
    }
  ...
  setup(): void {
    ...
    fields.forEach((field, f) => {
      ...
      const inputContainer = this.scene.add.container(
          inputPos.x,
          hasTitle ? inputPos.y : inputPos.y - 26 + (20 * f)
      );
      ...
      const inputConfig = this.getInputConfig(field);
      const input = addTextInputObject(this.scene, 4, -2, 440, 116, TextStyle.TOOLTIP_CONTENT, inputConfig);
      input.setOrigin(0, 0);
