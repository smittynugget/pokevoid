...
export default class AbilityBar extends Phaser.GameObjects.Container {
  ...
  showAbility(pokemon: Pokemon, passive: boolean = false, ability?: Ability): void {
    const abilityName = ability ? ability.name : !passive ?  pokemon.getAbility().name : pokemon.getPassiveAbility().name;
    this.abilityBarText.setText(`${i18next.t("fightUiHandler:abilityFlyInText", { pokemonName: getPokemonNameWithAffix(pokemon), passive: passive ? i18next.t("fightUiHandler:passive") : "", abilityName: abilityName })}`);
