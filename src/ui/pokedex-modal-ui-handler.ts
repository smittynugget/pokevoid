...
export default class PokedexModalUiHandler extends ModalUiHandler {
    ...
    private destroyOwnedPokemonInstance(): void {
        if (this.ownedPokemonInstance) {
            try { this.ownedPokemonInstance.destroy(); } catch {}
    ...
    private async loadPokemonSprite(speciesId: Species, formIndex: number = 0): Promise<void> {
        ...
        try {
            ...
            if (isGlitchOrSmittyForm && formName) {
                ...
                const glitchSpriteKey = form.getSpriteKey(false);
                const glitchAtlasPath = form.getSpriteAtlasPath(false);
                const glitchFileName = glitchAtlasPath.startsWith("glitch/")
                    ? glitchAtlasPath.slice("glitch/".length)
                    : sanitizedFormName;
                spriteKey = glitchSpriteKey;

                if (!this.scene.textures.exists(spriteKey)) {
                    try {
                        await this.loadGlitchSpriteFromFile(glitchFileName, spriteKey);
                    } catch (fileLoadError) {
