...
export default class RewardObtainedUiHandler extends ModalUiHandler {
    ...
    if (this.rewardConfig.isMod) {
      ...
      try {
        ...
        if (storedMod && storedMod.spriteData) {
          ...
          return new Promise((resolve, reject) => {
            const spriteData = storedMod.spriteData;
  ...
  private setupModifierSprite(): void {
    ...
    if (this.rewardConfig.modifierType.group === "trainer") {
      ...
      const itemAtlas = this.rewardConfig.modifierType.group === "glitch" || this.rewardConfig.modifierType.group === "perma" ? "smitems" : "items";
  ...
  show(args: any[]): boolean {
    ...
    if (args.length >= 2 && "buttonActions" in args[0]) {
      ...
      if (super.show(args)) {
        ...
        this.loadTexture()
          .then(() => {
            if (!this.active || !this.uiContainer) {
              return;
            }
            this.setupSprite();
            this.setupUI();
  ...
  protected setupUI(): void {
    if (!this.uiContainer || !this.modalContainer) {
      return;
    }
