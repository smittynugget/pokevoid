...
export class SwitchSummonPhase extends SummonPhase {
  ...
  preSummon(): void {
    if (!this.player) {
      if (this.slotIndex === -1) {
        //@ts-ignore
        this.slotIndex = this.scene.currentBattle.trainer?.getNextSummonIndex( TrainerSlot.TRAINER); // TODO: what would be the default trainer-slot fallback?
      }
      if (this.slotIndex > -1) {
    ...
    if (this.scene.gameMode.isNuzlockeActive(this.scene)) {
      this.scene.getParty().forEach((pokemon, index) => {
        if (pokemon.isFainted()) {
          ...
          pokemon.destroy();
          // this.scene.getParty().splice(index, 1);
        }
      });
    }
