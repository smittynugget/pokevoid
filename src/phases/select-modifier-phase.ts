...
export class SelectModifierPhase extends BattlePhase {
  ...
  start() {
    ...
    const shouldRegeneratePool = this.getPoolType() !== null &&
            (!this.pathNodeFilter || this.pathNodeFilter === PathNodeTypeFilter.NONE);
  ...
  getRerollCost(typeOptions: ModifierTypeOption[], lockRarities: boolean): { rerollCost: number; permaRerollCost: number } {
    ...
    if (!isCollectedTypeShop && Utils.randSeedInt(100, 1) <= 5) {
      ...
      const wave = this.draftOnly && this.scene.gameMode.isNightmare ? this.scene.currentBattle.waveIndex % 100 : this.scene.currentBattle.waveIndex;
      this.cachedRerollCost = Math.min(Math.ceil(wave / 10) * baseValue * Math.pow(2, this.rerollCount), Number.MAX_SAFE_INTEGER);
    }
    const hasFilter = this.pathNodeFilter !== PathNodeTypeFilter.NONE;
  ...
  getModifierTypeOptions(modifierCount: integer): ModifierTypeOption[] {
    ...
    if (this.scene.moveUpgradesEnabledForRun) {
      return getPlayerModifierTypeOptions(modifierCount, this.scene.getParty(), this.scene.lockModifierTiers ? this.modifierTiers : undefined, this.draftOnly, effectivePathNodeFilter);
    }

    const baseOptions = getPlayerModifierTypeOptions(modifierCount + 4, this.scene.getParty(), this.scene.lockModifierTiers ? this.modifierTiers : undefined, this.draftOnly, effectivePathNodeFilter);
    ...
    while (filtered.length < modifierCount && attempts < 25) {
      const extra = getPlayerModifierTypeOptions(1, this.scene.getParty(), this.scene.lockModifierTiers ? this.modifierTiers : undefined, this.draftOnly, effectivePathNodeFilter);
      const next = extra[0];
