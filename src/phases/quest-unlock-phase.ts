            ...
            !this.isQuestInitialUnlock && (isGlitchForm || isSmittyForm) ?
                {
                    ...
                    name: isGlitchForm
                        ? (getPokemonSpecies(rewardId as Species).getGlitchFormName(true, undefined, this.questUnlockData.rewardType) ?? "unknown").toLowerCase()
                        : (getPokemonSpecies(rewardId as Species).getSmittyFormName(true) ?? "unknown").toLowerCase(),
