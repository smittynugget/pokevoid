...
export class TitlePhase extends Phase {
  ...
  showOptions(): void {
    ...
    const introTutorials = [EnhancedTutorial.LEGENDARY_POKEMON_1, EnhancedTutorial.BUG_TYPES_1];
    const firstVictoryTutorials = [EnhancedTutorial.FIRST_VICTORY, EnhancedTutorial.NEW_QUESTS];
    const bountiesTutorials = [EnhancedTutorial.DAILY_BOUNTY, EnhancedTutorial.BOUNTIES_1];
    const menuAccess = [EnhancedTutorial.MENU_ACCESS, EnhancedTutorial.STATS, EnhancedTutorial.RUN_HISTORY_1];
    const testDetails = [EnhancedTutorial.EGG_SWAP_1, EnhancedTutorial.EGGS_1, EnhancedTutorial.UNLOCK_JOURNEY];
    const v2UpdateTutorials = [
  ...
  private shouldShowTitleCutscene(): boolean {
    ...
    if (allSmittysComplete) {
      return this.scene.gameData.gameStats.cutsceneTitleDShown === false;
    }
    if (voidOvertaken) {
      return this.scene.gameData.gameStats.cutsceneTitleCShown === false;
    }
    if (nightmareUnlocked) {
      return this.scene.gameData.gameStats.cutsceneTitleBShown === false;
    }
  ...
  private triggerTitleCutscene(): void {
    ...
    if (allSmittysComplete) {
      ...
      flagSetter = () => {
        this.scene.gameData.gameStats.cutsceneTitleDShown = true;
      };
    } else if (voidOvertaken) {
      def = STORY_CUTSCENES.title_intro_c;
      flagSetter = () => {
        this.scene.gameData.gameStats.cutsceneTitleCShown = true;
      };
