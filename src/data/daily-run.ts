...
export function getDailyRunStarters(scene: BattleScene, seed: string): Starter[] {
  ...
  scene.executeWithSeedOffset(() => {
    ...
    for (let c = 0; c < starterCosts.length; c++) {
      ...
      const starterSpecies = getPokemonSpecies(randPkmSpecies.getTrainerSpeciesForLevel(startingLevel, true, PartyMemberStrength.STRONGER, scene.currentBattle.waveIndex, scene.gameMode.isNightmare));
