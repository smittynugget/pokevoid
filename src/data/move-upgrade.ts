...
export class MoveUpgrade {
    ...
    static generateMoveUpgradeOptions(moveId: Moves, scene: BattleScene, isPlayer: boolean = true, lowTierUpgrade: boolean = false): ModifierType[] {
        ...
        const selfLowerAttrs = baseMove.getAttrs(StatChangeAttr).filter((a: StatChangeAttr) => a.selfTarget && a.levels < 0);
        ...
        if (shouldOfferUpgradeCategory(UpgradeCategory.STAT_BOOST_SELF) && hasStatBoostSelf && selfBoostAttr && hasPower && !isMultiHit) {
            ...
            if (!hasPathlessUpgrade && isStatusMove && !isMultiHit && isRare) {
        ...
        if (shouldOfferUpgradeCategory(UpgradeCategory.ACCURACY) && hasAccuracy && upgradeMoveAccuracy < 100 && !isStatusMove && hasPower) {
            ...
            if (upgradeTier !== null) {
                ...
                if (nextStep) {
                    const accDelta = nextStep.accBoost === 101 ? (101 - baseMoveAccuracy) : nextStep.accBoost;
                    const descKey = nextStep.accBoost === 101 ? "moveUpgrade:description:accuracy:perfectAccuracy" : "moveUpgrade:description:accuracy:increaseVsPower";
                    const newPower = baseMovePower - nextStep.pCost;
                    const newAccuracy = nextStep.accBoost === 101 ? 101 : (baseMoveAccuracy === -1 ? (100 + nextStep.accBoost) : (baseMoveAccuracy + nextStep.accBoost));
            ...
            if (upgradeTier !== null) {
                ...
                if (nextStep) {
                    ...
                    upgrades.push(moveGenerator.getType(moveId, powerDelta, null, null, 0,
                        ...
                        }), chance, null, chosen.attr, [], 0, UpgradeCategory.EFFECT_CHANCE, upgradeTier));
                }
        }
        }
        ...
        if(!hasPathlessUpgrade && isVeryRare) {
        ...
        if (!isStatusMove) {
            ...
            if (!baseMove.hasAttr(WeatherBallTypeAttr) && !baseMove.hasAttr(MatchUserTypeAttr) &&
                !baseMove.hasAttr(TerrainPulseTypeAttr) && !baseMove.hasAttr(HiddenPowerTypeAttr)) {
                upgrades.push(moveGenerator.getType(moveId, 0, null, null, 0,
            ...
            if (!baseMove.hasAttr(TerrainPulseTypeAttr) && !baseMove.hasAttr(MatchUserTypeAttr) &&
            ...
            if (!baseMove.hasAttr(HiddenPowerTypeAttr) && !baseMove.hasAttr(MatchUserTypeAttr) &&
        ...
        if (hasPower && !isStatusMove && upgradeMovePower <= 95) {
        ...
        if (!isStatusMove && upgradeMovePower <= 60 && !baseMove.hasAttr(WeatherPowerBoostAttr) && hasPower) {
            const weather = Utils.randSeedItem([WeatherType.SUNNY, WeatherType.RAIN, WeatherType.SANDSTORM, WeatherType.SNOW]);
        ...
        if (!isStatusMove && upgradeMovePower <= 60 && !baseMove.hasAttr(TerrainPowerBoostAttr) && hasPower) {
            const terrain = Utils.randSeedItem([TerrainType.ELECTRIC, TerrainType.GRASSY, TerrainType.MISTY, TerrainType.PSYCHIC]);
        ...
        if (upgradeMove.priority <= 0 && !isStatusMove && !upgradeMove.hasAttr(TerrainMovePriorityAttr) && !upgradeMove.hasAttr(FirstTurnPriorityAttr) && hasPower) {
            ...
            if (!baseMove.hasAttr(SurviveDamageAttr) && hasPower) {
                upgrades.push(moveGenerator.getType(moveId, 25, null, null, 0,
            ...
            if (!baseMove.hasAttr(FixedDamageAttr) && !baseMove.hasAttr(LevelDamageAttr) &&
        ...
        if (!hasPathlessUpgrade && upgradeStatusAttr && !isStatusMove && upgradeMove.chance >= 5 && !isSelfTarget && isRare) {
        ...
        if (!hasPathlessUpgrade && isStatusMove && !upgradeStatusAttr && !hasStatBoostSelf && !hasStatLowerTarget && !hasHealAttr && !isSelfTarget && isRare) {
                ...
                const accuracyDelta = targetAcc - baseMoveAccuracy;
                const statusName = getStatusEffectName(status);
