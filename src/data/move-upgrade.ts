    ...
    if (shouldOfferUpgradeCategory(UpgradeCategory.EFFECT_CHANCE) && hasPower && !isStatusMove && !hasSecondaryEffect && !isMultiHit && !selfLowerAttrs.length) {
      ...
      if (upgradeTier !== null) {
        ...
        if (nextStep) {
          ...
          const status = isVeryRare ? Utils.randSeedItem([StatusEffect.BURN, StatusEffect.PARALYSIS, StatusEffect.POISON]) : Utils.randSeedItem([StatusEffect.BURN, StatusEffect.PARALYSIS, StatusEffect.POISON, StatusEffect.SLEEP, StatusEffect.FREEZE, StatusEffect.TOXIC]);
    ...
    if (shouldOfferUpgradeCategory(UpgradeCategory.CHARGE_MOVE) && !isStatusMove && !isMultiHit && hasPower && baseMovePower <= 100 && !selfLowerAttrs.length) {
      ...
      if (upgradeTier !== null && (upgradeTier > 1 || isRare)) {
        ...
        if (nextStep) {
          ...
          const attrs: MoveAttr[] = [new ChargeAttr(chargeAnim, chargeText, null, !!nextStep.addBoost)];
