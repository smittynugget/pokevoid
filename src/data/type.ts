...
export function setChangeNormalTyping(value: boolean) {
  changeNormalTyping = value;
}

export function getTypeDamageMultiplier(attackType: Type, defType: Type): TypeDamageMultiplier {

  if(nightmareTypeInteractions[defType] || nightmareTypeInteractions[attackType]) {
    if(nightmareTypeInteractions[attackType]) {
      console.log('nightmare type interaction', attackType, defType);
    }
    return calculateNightmareInteraction(attackType, defType);
  }
  ...
  case Type.POISON:
    if (attackType == Type.NORMAL && changeNormalTyping) {
        return 0.5;
    }
    switch (attackType) {
    case Type.GROUND:
...
const nightmareTypeInteractions: Partial<Record<Type, TypeInteractions>> = {};

export function setNightmareTypeInteractions(nightmareType: Type, interactions: TypeInteractions) {
  nightmareTypeInteractions[nightmareType] = interactions;
}

export function getMinNightmareTypeValue() {
  return 50;
}

function calculateNightmareInteraction(attackType: Type, defType: Type): TypeDamageMultiplier {
