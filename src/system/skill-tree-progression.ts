...
export class SkillTreeProgression {
	...
	canLevelUpTree(): boolean {
		...
		if (!activeSkillTree) return false;
	...
	awardTokens(amount: number, source: string): void {
		const activeSkillTree = this.scene.gameData.activeSkillTree;
		console.log(`[SKILL_TREE_DIAG] awardTokens called: amount=${amount}, source=${source}, activeSkillTree=${!!activeSkillTree}`);
		if (!activeSkillTree) {
			console.warn(`[SKILL_TREE_DIAG] awardTokens ABORTED: activeSkillTree is null/undefined`);
			return;
		}
		const oldTokens = activeSkillTree.tokens || 0;
		activeSkillTree.tokens = Math.max(0, oldTokens + amount);
		console.log(`[SKILL_TREE_DIAG] awardTokens SUCCESS: ${oldTokens} -> ${activeSkillTree.tokens} tokens`);
	...
	awardSkillPoints(amount: number, source: string): void {
		const activeSkillTree = this.scene.gameData.activeSkillTree;
		console.log(`[SKILL_TREE_DIAG] awardSkillPoints called: amount=${amount}, source=${source}, activeSkillTree=${!!activeSkillTree}`);
		if (!activeSkillTree) {
			console.warn(`[SKILL_TREE_DIAG] awardSkillPoints ABORTED: activeSkillTree is null/undefined`);
			return;
		}
		const oldPoints = activeSkillTree.skillPoints || 0;
		activeSkillTree.skillPoints = Math.max(0, oldPoints + amount);
		console.log(`[SKILL_TREE_DIAG] awardSkillPoints SUCCESS: ${oldPoints} -> ${activeSkillTree.skillPoints} skill points`);
		this.scene.unshiftPhase(new RewardObtainDisplayPhase(this.scene, {
