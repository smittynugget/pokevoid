...
const REWARD_OVERLAY_OPACITY_OPTIONS: SettingOption[] = new Array(9).fill(null).map((_, i) => {
  const value = ((i + 1) * 10).toString();
  return {
    value,
    label: value,
...
export const Setting: Array<Setting> = [
  ...
  {
    key: SettingKeys.Show_Item_Text_BG,
    label: i18next.t("settings:showItemTextBg"),
    options: OFF_ON,
    default: 1,
    type: SettingType.DISPLAY
  },
  ...
  {
    key: SettingKeys.Reward_Overlay_Opacity,
    label: i18next.t("settings:rewardOverlayOpacity"),
    options: REWARD_OVERLAY_OPACITY_OPTIONS,
    default: 8,
    type: SettingType.DISPLAY,
    requireReload: false
  },
...
export function setSetting(scene: BattleScene, setting: string, value: integer): boolean {
  ...
  case SettingKeys.Shop_Overlay_Opacity:
    ...
    break;
  case SettingKeys.Reward_Overlay_Opacity:
    scene.gameData?.updateRewardOverlayOpacity(parseInt(Setting[index].options[value].value) * .01);
    break;
  }
