...
import {allSpecies, getPokemonSpecies, universalSmittyForms, isGlitchFormKey, speciesStarters} from "../data/pokemon-species";
...
import { getFormChangeItemSpriteFrame } from "#app/utils/form-change-item-sprite-utils";
...
import {GameData, QuestUnlockData} from "#app/system/game-data";
...
import type { FilterUpgrades } from "../data/move-upgrade";
import { UpgradePath } from "#enums/upgrade-path";
import { UpgradeCategory } from "../enums/upgrade-category";
import { PokemonAltBuildDefinition, POKEMON_ALT_BUILDS, PokemonAltBuildId } from "../data/pokemon-alt-buid";
import { PlayableChampionData } from "../system/playable-champions";
import { ChampionUtils } from "#app/system/champion-utils.js";
import { PlayerGender } from "#enums/player-gender";
import { SkillTreeSelectors } from "../system/skill-tree-selectors";
...
function getUpgradeCategoryIcon(upgradeCategory?: UpgradeCategory): string {
    if (!upgradeCategory) {
        return "smittyHumor";
    }
...
export class ModifierType {
    ...
    constructor(localeKey: string | null, iconImage: string | null, newModifierFunc: NewModifierFunc | null, group?: string, soundName?: string, isInverted?: boolean) {
...
export type ForbiddenFormUnlockCandidate =
    | {
          kind: "QUEST_FORM";
...
export const FORBIDDEN_FORM_REWARDTYPE_TO_FORMKEY: Partial<Record<RewardType, SpeciesFormKey>> = {
    ...
    [RewardType.SMITTY_FORM]: SpeciesFormKey.SMITTY,
    [RewardType.SMITTY_FORM_B]: SpeciesFormKey.SMITTY_B
};

function normalizeGlitchOrSmittyAssetKey(formName: string): string {
    const raw = (formName || "").toLowerCase();

    if (raw === "porygonω") return "porygon-q";
    if (raw === "karasu-me") return "karasume";
    return raw;
}

export class ForbiddenFormUnlockModifierType extends ModifierType {
    public readonly candidate: ForbiddenFormUnlockCandidate;
    public readonly iconAtlasKey: string;
    public readonly iconFrame: string | null;

    constructor(candidate: ForbiddenFormUnlockCandidate) {
        super(null, "pb", (type, _args) => new Modifiers.ForbiddenFormUnlockModifier(type, candidate), "forbiddenFormUnlock", "se/restore");
        this.candidate = candidate;
        this.setTier(ModifierTier.LUXURY);

        const icon = this.resolveIcon();
        this.iconAtlasKey = icon.key;
        this.iconFrame = icon.frame;
    }

    get name(): string {
        const label = this.getFormDisplayName();
        return label || i18next.t("skillTree:fallback.unknownGlitchForm");
    }

    getDescription(_scene: BattleScene): string {
        const data = this.getTooltipData();
        if (data?.isSmitty) {
            return i18next.t("skillTree:descriptions.smittyFormTooltipDescription");
        }
        ...
        if (species && form) {
            ...
            return i18next.t("skillTree:descriptions.forbiddenFormUnlockRunIntro", { species, form, item });
        }
        return i18next.t("skillTree:descriptions.randomGlitchFormsForRun");
    }

    public getTooltipData(): {
        kind: ForbiddenFormUnlockCandidate["kind"];
        ...
        baseStats?: number[];
        targetStats?: number[];
        formChangeItem?: FormChangeItem | null;
    } | null {
        const resolved = this.resolveFormPreview();
        if (!resolved) return null;

        const types = [resolved.type1, resolved.type2].filter(t => t !== null && t !== undefined) as Type[];
        const abilities = [resolved.ability1, resolved.ability2, resolved.abilityHidden]
            .filter(a => typeof a === "number" && a >= 0) as Abilities[];

        const c: any = this.candidate as any;
        let speciesId: Species | null = null;
        let formKey: SpeciesFormKey | null = null;
        let formIndex: integer | null = null;
        let formName: string | null = null;

        if (c?.kind === "MOD_FORM") {
            speciesId = c.speciesId as Species;
            try {
                const species = getPokemonSpecies(speciesId);
                const form = species?.forms?.find((f: any) => (f?.formName || "").toLowerCase() === (c.formName || "").toLowerCase());
                formKey = (form?.formKey as SpeciesFormKey) ?? null;
                formIndex = (typeof form?.formIndex === "number" ? (form.formIndex as any) : null);
                if (formIndex !== null && typeof species?.getName === "function") {
                    formName = species.getName(formIndex);
                } else {
                    formName = (getModPokemonName(speciesId, c.formName) || c.formName || null);
                }
            } catch {
                formName = (c.formName || null);
            }
        } else if (c?.kind === "QUEST_FORM") {
            try {
                speciesId = this.resolveQuestRepresentativeSpeciesId(c.questUnlockData);
                const rt = c.questUnlockData?.rewardType as RewardType;
                formKey = FORBIDDEN_FORM_REWARDTYPE_TO_FORMKEY[rt] ?? null;
                if (speciesId != null) {
                    const species = getPokemonSpecies(speciesId);
                    const form = this.resolveSpeciesFormForRewardType(speciesId, rt);
                    formIndex = (typeof form?.formIndex === "number" ? (form.formIndex as any) : null);
                    if (formIndex !== null && typeof species?.getName === "function") {
                        formName = species.getName(formIndex);
                    } else {
            ...
            try {
                const uni = universalSmittyForms.find(f => (f?.formName || "").toLowerCase() === (c.formName || "").toLowerCase());
                formKey = (uni?.formKey as SpeciesFormKey) ?? null;
            } catch {
            }
            try {
                const key = normalizeGlitchOrSmittyAssetKey(c.formName || "");
                const t = i18next.t(`smittyNames:${key}.name`, { defaultValue: c.formName || "" });
                formName = (t && typeof t === "string") ? t : (c.formName || null);
            } catch {
                formName = (c.formName || null);
            }
        }
        ...
        try {
            if (speciesId != null) {
                const species = getPokemonSpecies(speciesId);
                speciesName = (typeof species?.getName === "function") ? species.getName() : (species?.name ?? null);
            }
        } catch {
        }

        let formChangeItem: FormChangeItem | null = null;
        try {
            if (speciesId != null) {
                const changes = pokemonFormChanges[speciesId] || [];
                let match: any | null = null;
                if (c?.kind === "MOD_FORM" && typeof c?.systemName === "string") {
                    match = changes.find((fc: any) => fc?.modFormName === c.systemName) || null;
                }
                if (!match && formKey != null) {
                    match = changes.find((fc: any) => {
                        if (fc?.formKey !== formKey) return false;

                        if (c?.kind === "QUEST_FORM" && typeof fc?.isModForm === "function" && fc.isModForm()) return false;
                        return true;
                    }) || null;
                }
                const trig = match?.findTrigger?.(SpeciesFormChangeItemTrigger) as any;
                formChangeItem = (typeof trig?.item === "number") ? (trig.item as FormChangeItem) : null;
            }
        } catch {
        }

        const isSmitty = c?.kind === "UNI_SMITTY" ||
            (formKey != null && (formKey === SpeciesFormKey.SMITTY || formKey === SpeciesFormKey.SMITTY_B));

        return {
            kind: this.candidate.kind,
            speciesId,
            speciesName,
            formKey,
            formName: formName || this.getFormDisplayName() || null,
            types,
            abilities,
            baseTotal: resolved.baseTotal,
            targetTotal: resolved.targetTotal,
            baseStats: resolved.baseStats,
            targetStats: resolved.targetStats,
            formChangeItem,
            isSmitty
        };
    }

    private resolveQuestRepresentativeSpeciesId(questUnlockData: QuestUnlockData): Species | null {
        const rid: any = questUnlockData?.rewardId as any;
    ...
    private resolveSpeciesFormForRewardType(speciesId: Species, rewardType: RewardType): any | null {
        try {
            const species = getPokemonSpecies(speciesId);
            if (!species?.forms?.length) return null;
            const formKey = FORBIDDEN_FORM_REWARDTYPE_TO_FORMKEY[rewardType];
            if (!formKey) return null;
            return species.forms.find((f: any) => f?.formKey === formKey) || null;
        } catch {
            return null;
        }
    }

    private getFormDisplayName(): string | null {
        const c = this.candidate;
        if (c.kind === "UNI_SMITTY") {
            try {
                const key = normalizeGlitchOrSmittyAssetKey(c.formName || "");
                const t = i18next.t(`smittyNames:${key}.name`, { defaultValue: c.formName || "" });
                return (t && typeof t === "string") ? t : (c.formName || null);
            } catch {
                return c.formName || null;
            }
        }
        if (c.kind === "MOD_FORM") {
            try {
                const species = getPokemonSpecies(c.speciesId);
                const form = species?.forms?.find((f: any) => (f?.formName || "").toLowerCase() === (c.formName || "").toLowerCase());
                if (form && typeof species?.getName === "function") {
                    return species.getName(form.formIndex);
                }
                const localized = getModPokemonName(c.speciesId, c.formName);
                return localized || c.formName || null;
            } catch {
                return c.formName || null;
            }
        }
        if (c.kind === "QUEST_FORM") {
            const rep = this.resolveQuestRepresentativeSpeciesId(c.questUnlockData);
            if (!rep) return null;
            const rewardType = c.questUnlockData.rewardType as RewardType;
            const form = this.resolveSpeciesFormForRewardType(rep, rewardType);
            try {
                const species = getPokemonSpecies(rep);
                if (typeof species?.getName === "function" && typeof form?.formIndex === "number") {
                    return species.getName(form.formIndex);
                }
            } catch {
            }
            return form?.formName || null;
        }
        return null;
    }

    private resolveIcon(): { key: string; frame: string | null } {
        const fallback = { key: "pokemon_icons_glitch", frame: "smitom" };
        const c = this.candidate;

        if (c.kind === "UNI_SMITTY") {
            return { key: "pokemon_icons_glitch", frame: normalizeGlitchOrSmittyAssetKey(c.formName || "smitom") };
        }
        ...
        if (c.kind === "MOD_FORM") {
            try {
                ...
                const form = species?.forms?.find((f: any) => (f?.formName || "").toLowerCase() === (c.formName || "").toLowerCase());
                if (form) {
                    const key = form.getIconAtlasKey?.() || fallback.key;
                    const frame = typeof form.getIconId === "function" ? form.getIconId(false) : fallback.frame;
                    return { key, frame };
                }
            } catch {
            }
    ...
    private resolveFormPreview(): {
        ...
        if (c.kind === "UNI_SMITTY") {
            ...
            if (!uni) return null;
            return {
                type1: (uni.primaryType ?? Type.NORMAL) as any,
                type2: (uni.secondaryType ?? null) as any,
                ability1: uni.ability1,
                ability2: uni.ability2,
                abilityHidden: uni.abilityHidden,
                targetTotal: uni.totalStats,
                targetStats: [uni.hp, uni.attack, uni.defense, uni.spAttack, uni.spDefense, uni.speed]
            };
        }
        ...
        if (c.kind === "MOD_FORM") {
            try {
                ...
                return {
                    ...
                    abilityHidden: form.abilityHidden,
                    baseTotal: species.baseTotal,
                    targetTotal: form.baseTotal,
                    baseStats: species.baseStats,
                    targetStats: form.baseStats
                };
            } catch {
                return null;
            }
        ...
        if (c.kind === "QUEST_FORM") {
            try {
                ...
                const species = getPokemonSpecies(rep);
                const rewardType = c.questUnlockData.rewardType as RewardType;
                const form: any = this.resolveSpeciesFormForRewardType(rep, rewardType);
                if (!form) return null;
                return {
                    type1: form.type1,
                    type2: form.type2,
...
export class AddTypeBallModifierType extends ModifierType {
    ...
    get name(): string {
        const displayName = i18next.t(`pokemonInfo:Type.${Type[this.targetType]}`, { defaultValue: Type[this.targetType] });
        return i18next.t("modifierType:ModifierType.AddPokeballModifierType.name", {
            "modifierCount": this.count,
    ...
    get name(): string {
        if (this.newPrimaryType != null && this.newSecondaryType != null) {
            return i18next.t("modifierType:ModifierType.TypeSwitcherModifierType.name", {
                type1: i18next.t(`pokemonInfo:Type.${Type[this.newPrimaryType]}`, { defaultValue: Type[this.newPrimaryType] }),
                type2: i18next.t(`pokemonInfo:Type.${Type[this.newSecondaryType]}`, { defaultValue: Type[this.newSecondaryType] })
            });
...
export class AbilitySwitcherModifierType extends PokemonHeldItemModifierType {
    ...
    getDescription(scene: BattleScene): string {
        return i18next.t("modifierType:ModifierType.AbilitySwitcherModifierType.description");
    }
}
...
export class AnyTmModifierType extends PokemonModifierType {
    ...
    constructor(moveId: Moves) {
        ...
        if (moveDetails.power > 0 || moveDetails.accuracy > 0) {
            ...
            description = `[${moveDetails.power > 0 ? `POW ${moveDetails.power} ` : ""}${moveDetails.accuracy > 0 ? `ACC ${moveDetails.accuracy} ` : ""}${moveCategory}] ${truncatedEffect}`;
        } else {
            truncatedEffect = truncatedEffect.length > 70 ? truncatedEffect.substring(0, 67) + "..." : truncatedEffect;
            description = `[${moveCategory}] ${truncatedEffect}`;
        }
    ...
    getDescription(scene: BattleScene): string {
        return i18next.t(scene.enableMoveInfo ? "modifierType:ModifierType.AnyTmModifierTypeWithInfo.description" : "modifierType:ModifierType.AnyTmModifierType.description", {moveName: allMoves[this.moveId].name});
    }
}
...
export const PARTY_INELIGIBLE_ABILITIES = [
    Abilities.IMPOSTER,
    Abilities.NORMALIZE,
...
export class AnyAbilityModifierType extends PokemonModifierType {
    ...
    getTooltipDescription(scene: BattleScene): string {
        return [this.getDescription(scene), "", this.ability.description].join("\n");
    }
}
            ...
            let randomAbility = abilities[randomIndex];
             ...
             if (PARTY_OP_ABILITIES.includes(randomAbility)) {
                const abilityAlreadyExists = party.some(pokemon => {
                    if (!pokemon || !pokemon.getAbility) return false;
                    const mainAbility = pokemon.getAbility();
                    const hasMainAbility = !!mainAbility && mainAbility.id === randomAbility;
                    const passiveAbility = pokemon.hasPassive() && pokemon.getPassiveAbility ? pokemon.getPassiveAbility() : undefined;
                    const hasPassiveAbility = !!passiveAbility && passiveAbility.id === randomAbility;
...
export class MoveUpgradeModifierType extends ModifierType implements GeneratedPersistentModifierType {
    ...
    constructor(
        ...
        super(
            ...
            getUpgradeCategoryIcon(upgradeCategory),
    ...
    get name(): string {
        const moveName = allMoves[this.moveId].name;
        const label = this.upgradeCategory
            ? i18next.t(`moveUpgradeAttrs:${this.upgradeCategory}`)
            : i18next.t("moveUpgradeAttrs:extraEffectUpgrade");
        const m = typeof label === "string" ? label.match(/\(([^)]+)\)/) : null;
        const flavor = m?.[1]
            ? m[1].toUpperCase()
            : (this.upgradeCategory ? String(this.upgradeCategory) : "EX");
        const rank = typeof this.upgradeTier === "number" ? Utils.intToRoman(this.upgradeTier) : "";
        return `${moveName} ${flavor}${rank ? ` ${rank}` : ""}`;
    }
...
export class MoveUpgradeModifierTypeGenerator extends ModifierTypeGenerator {
    private lowTierUpgrade: boolean;
    private filterUpgrades?: FilterUpgrades;
    constructor(lowTierUpgrade: boolean = false, filterUpgrades?: FilterUpgrades) {
        super((party: Pokemon[], pregenArgs?: any[]) => {
            if (pregenArgs && pregenArgs.length >= 1) {
                ...
                const upgradeCategory = pregenArgs[11] as UpgradeCategory || undefined;
                const upgradeTier = pregenArgs[12] as number || undefined;
            ...
            if (pregenArgs && pregenArgs.length > 13) {
                const maybeFilter = pregenArgs[13] as FilterUpgrades | undefined;
                if (maybeFilter) this.filterUpgrades = maybeFilter;
            }
            return this.generateRandomMoveUpgrade(party);
        });
        this.lowTierUpgrade = lowTierUpgrade;
        this.filterUpgrades = filterUpgrades;
    }

    private generateRandomMoveUpgrade(party: Pokemon[]): MoveUpgradeModifierType | null {
        const availablePokemon = party.filter(pokemon =>
            pokemon && pokemon.getMoveset().some(move => move && !move.virtual)
        );
        if (availablePokemon.length === 0) {
            return null;
        }
        const scene = availablePokemon[0].scene;
        const isEnemy = !availablePokemon[0].isPlayer();
        if(!isEnemy && scene.enemyMovesUpgradedForWave.wave !== scene.currentBattle.waveIndex) {
        }
        if(isEnemy && scene.enemyMovesUpgradedForWave.wave !== scene.currentBattle.waveIndex) {
            scene.enemyMovesUpgradedForWave.wave = scene.currentBattle.waveIndex;
            scene.enemyMovesUpgradedForWave.moves = [];
        }
        const pairs: { p: Pokemon; m: any }[] = [];
        for (const p of availablePokemon) {
            for (const m of p.getMoveset()) {
                if (!m || m.virtual) continue;
                if (isEnemy && scene.enemyMovesUpgradedForWave.moves.includes(m.moveId)) continue;
                pairs.push({ p, m });
            }
        }
        ...
        const eligible: { isPlayer: boolean; moveId: Moves; options: ModifierType[] }[] = [];
        for (const { p, m } of pairs) {
            const isPlayer = p.isPlayer();
            const upgradeOptions = MoveUpgrade.generateMoveUpgradeOptions(m.moveId, scene, isPlayer, this.lowTierUpgrade, this.filterUpgrades);
            if (upgradeOptions.length > 0) {
                eligible.push({ isPlayer, moveId: m.moveId, options: upgradeOptions });
            }
        }
        if (eligible.length > 0) {
            const picked = Utils.randSeedItem(eligible) as { isPlayer: boolean; moveId: Moves; options: ModifierType[] };
            if (!picked.isPlayer) {
                scene.enemyMovesUpgradedForWave.moves.push(picked.moveId);
            }
            return Utils.randSeedItem(picked.options) as MoveUpgradeModifierType;
        }
        return null;
    }

    getType(moveId?: Moves, powerBoost?: integer, typeChange?: Type, categoryChange?: MoveCategory, accuracyBoost?: integer,
            effectChange?: string, chanceChange?: integer, moveTargetChange?: MoveTarget, attrs?: MoveAttr[], conditions?: MoveCondition[], flags?: integer, upgradeCategory?: UpgradeCategory, upgradeTier?: number): ModifierType {
        if (moveId === undefined) {
            return this;
        }
        ...
        return new MoveUpgradeModifierType(
            moveId,
            powerBoost || 0,
            typeChange !== undefined ? typeChange : null,
            categoryChange !== undefined ? categoryChange : null,
            accuracyBoost || 0,
            effectChange || null,
            chanceChange !== undefined ? chanceChange : null,
            moveTargetChange !== undefined ? moveTargetChange : null,
            attrs || [],
            conditions || [],
            flags || 0,
            upgradeCategory,
...
export function createModifierTypeOption(modifierTypeFunc: ModifierTypeFunc, upgradeCount: number, cost: number = 500, scene?: BattleScene): ModifierTypeOption {
    ...
    const getPermaModifiersByRarity = (rarity: number): ModifierTypeOption[] => {
        return Object.entries(modifierTypes)
            .filter(([key, factory]) => {
                ...
                if (key === 'PERMA_COLLECTED_TYPE') return false;
            ...
            let abilityDetails = new Ability(abilities[randomIndex] as Abilities, 0);
             ...
             if (PARTY_OP_ABILITIES.includes(abilityDetails.id)) {
                const abilityAlreadyExists = party.some(pokemon => {
                    if (!pokemon || !pokemon.getAbility) return false;
                    const mainAbility = pokemon.getAbility();
                    const hasMainAbility = !!mainAbility && mainAbility.id === abilityDetails.id;
                    const passiveAbility = pokemon.hasPassive() && pokemon.getPassiveAbility ? pokemon.getPassiveAbility() : undefined;
                    const hasPassiveAbility = !!passiveAbility && passiveAbility.id === abilityDetails.id;
                    return hasMainAbility || hasPassiveAbility;
                });
...
export class StatSacrificeModifierType extends PokemonHeldItemModifierType {
    ...
    getDescription(scene: BattleScene): string {
        return i18next.t("modifierType:ModifierType.StatSacrificeModifierType.description", { stat: getStatName(this.stat) }) +
               "\n" + i18next.t("modifierType:common.essenceAlternativeCost");
    }
}
...
export class PassiveAbilitySacrificeModifierType extends PokemonHeldItemModifierType {
    ...
    get name(): string {
        return i18next.t("modifierType:ModifierType.PassiveAbilitySacrificeModifierType.name");
    }
...
export class AddPokemonModifierTypeGenerator extends ModifierTypeGenerator {
    ...
    constructor(isDraft: boolean = false) {
        super((party: Pokemon[], pregenArgs?: any[]) => {
            ...
            const duelmonBstLimit = getDuelmonBstLimitForWave(currentWave);
            const allRandomPokemon = allSpecies.filter(s => {
                if (s.speciesId === Species.ETERNATUS) return false;
                if (s.generation === 20) {
                    if (!scene.duelmonsEnabledForRun) return false;
                    if (!speciesStarters.hasOwnProperty(s.speciesId)) return false;
                    if (s.baseTotal > duelmonBstLimit) return false;
                    return true;
                }

                return legendOrCatchableCheck
                    ? !pokemonPrevolutions.hasOwnProperty(s.speciesId)
                    : (!s.legendary && !s.mythical && !s.subLegendary) && s.isCatchable() && !pokemonPrevolutions.hasOwnProperty(s.speciesId) && s.speciesId !== Species.BLOODMOON_URSALUNA;
            });
...
export class CollectedTypeModifierType extends PokemonHeldItemModifierType implements GeneratedPersistentModifierType {
    ...
    constructor(collectedType: Type, playerPokemon?: PlayerPokemon) {
        super(
            ...
            "modSoulCollected",
            (_type, args) => {
                const pokemonId = playerPokemon ? playerPokemon.id : args[0] instanceof PlayerPokemon ? args[0].id : args[0];

                if (args.length >= 2) {
                    if (typeof args[1] === 'object' && args[1] !== null) {
                        return new CollectedTypeModifier(_type, pokemonId, args[1]);
                    } else {
                        return new CollectedTypeModifier(_type, pokemonId, args[1]);
                    }
                } else {
                    return new CollectedTypeModifier(_type, pokemonId, collectedType);
                }
            },
            'glitch'
        );
    ...
    get name(): string {
        ...
        const typeName = i18next.t(`pokemonInfo:Type.${Type[this.collectedType]}`);

        if (scene?.ui?.getMode) {
            const mode = scene.ui.getMode();
            if (mode === Mode.SHOP_SELECT || mode === Mode.MODIFIER_SELECT || mode === Mode.LOOT_REWARD_SELECT || mode === Mode.COLLECTED_TYPE_SELECT || scene.currentPhase?.constructor.name === "ModifierRewardPhase") {
                return i18next.t("modifierType:ModifierType.CollectedTypeModifierType.shopName", { type: typeName });
            }
        }

        return i18next.t("modifierType:ModifierType.CollectedTypeModifierType.name");
    }

    getTooltipDescription(scene: BattleScene): string {
        const typeName = i18next.t(`pokemonInfo:Type.${Type[this.collectedType]}`);
        return i18next.t("modifierType:ModifierType.CollectedTypeModifierType.tooltipDescription", { type: typeName });
    }

    getDescription(scene: BattleScene): string {
        const typeName = i18next.t(`pokemonInfo:Type.${Type[this.collectedType]}`);
        const baseDescription = i18next.t("modifierType:ModifierType.CollectedTypeModifierType.description", { type: typeName });
        ...
        let collectedTypesDisplay = "";
        const existingModifiers = scene.findModifiers(m =>
            m instanceof Modifiers.CollectedTypeModifier
        ) as Modifiers.CollectedTypeModifier[];

        if (existingModifiers.length > 0) {
            const allCollectedTypes: Partial<Record<Type, number>> = {};

            for (const modifier of existingModifiers) {
                for (const [type, count] of Object.entries(modifier.collectedTypes)) {
                    const typeKey = parseInt(type) as Type;
                    allCollectedTypes[typeKey] = (allCollectedTypes[typeKey] || 0) + count;
                }
            }

            const typeEntries = Object.entries(allCollectedTypes)
                .map(([type, count]) => `${Type[parseInt(type)]} x${count}`)
                .join(", ");

            if (typeEntries) {
                collectedTypesDisplay = ` [${typeEntries}]`;
            }
        }

        return baseDescription + collectedTypesDisplay;
    }
...
export class PermaCollectedTypeModifierType extends ModifierType {
    constructor() {
        super(
            "modifierType:ModifierType.PermaCollectedTypeModifierType",
            "modSoulCollected",
            (_type, args) => {
                if (args.length >= 1 && typeof args[0] === 'object' && args[0] !== null) {
                    return new PermaCollectedTypeModifier(_type, args[0]);
                } else {
                    return new PermaCollectedTypeModifier(_type);
                }
            },
            'glitch'
        );
        this.id = "PERMA_COLLECTED_TYPE";
    }

    get name(): string {
        return i18next.t("modifierType:ModifierType.PermaCollectedTypeModifierType.name", { defaultValue: "Collected Essences" });
    }

    getDescription(scene: BattleScene): string {
        const baseDescription = i18next.t("modifierType:ModifierType.PermaCollectedTypeModifierType.description", { defaultValue: "Stores collected essences:" });

        const permaModifier = scene.gameData.permaModifiers.getModifiers().find((m: any) => m.constructor.name === 'PermaCollectedTypeModifier') as any;
        let collectedTypesDisplay = "";
...
export function GeneratorInstanceCheck(generator: ModifierTypeGenerator): boolean {
    return (
        ...
        generator instanceof AnySmittyAbilityModifierTypeGenerator ||
        generator instanceof AnySmittyPassiveAbilityModifierTypeGenerator ||
        ...
        generator instanceof ChampionPokemonStatBoosterModifierTypeGenerator
    );
}
...
function glitchPieceWeightValAdjustment(party: Pokemon[], value: integer): integer {
    const scene = party[0]?.scene;
    if (!scene) return 0;
    const glitchModifier = scene.findModifier(m => m instanceof Modifiers.GlitchPieceModifier) as Modifiers.GlitchPieceModifier;
...
export class PermaPartyAbilityModifierType extends ModifierType {
    ...
    constructor(abilityID: Abilities) {
        ...
        this.setTier(ModifierTier.MASTER);
    }

    get name(): string {
        return i18next.t("modifierType:ModifierType.PermaModifierType.PERMA_PARTY_ABILITY.name", { abilityName: this.ability?.name ?? "Unknown" })
    }

    getDescription(scene: BattleScene): string {
        return i18next.t("modifierType:ModifierType.PermaModifierType.PERMA_PARTY_ABILITY.description", {
            abilityName: this.ability?.name ?? "Unknown"
        }) + i18next.t("modifierType:common.moreInfo");
    }

    getTooltipDescription(scene: BattleScene): string {
        return [
            i18next.t("modifierType:ModifierType.PermaModifierType.PERMA_PARTY_ABILITY.description", {
                abilityName: this.ability?.name ?? "Unknown"
            }),
            "",
    ...
    getTooltipLore(_scene: BattleScene): string {
        const durationLine = i18next.t("modifierType:permaWaveTooltip", { count: 30 });
        ...
        super((party: Pokemon[], pregenArgs?: any[]) => {
            ...
            const abilities = Object.values(allAbilities)
                .filter(ability =>
                    ...
                    !PARTY_INELIGIBLE_ABILITIES.includes(ability.id) &&
...
export class PermaModifierType extends ModifierType {
    ...
    const rank = getPermaModifierRarity(permaTypeKey);
    if (rank <= 1) this.setTier(ModifierTier.ULTRA);
    else if (rank === 2) this.setTier(ModifierTier.ROGUE);
    else this.setTier(ModifierTier.LUXURY);
    }
    ...
    getTooltipDescription(scene: BattleScene): string {
        return this.getDescription(scene);
    }

    getTooltipLore(_scene: BattleScene): string {
        const durationLine = this.permaDuration === PermaDuration.WAVE_BASED
            ? i18next.t("modifierType:permaWaveTooltip", { defaultValue: "Disappears after {{count}} waves \u2014 countdown starts after wave 50", count: this.count })
            : i18next.t("modifierType:permaUseTooltip", { defaultValue: "{{count}} uses remaining \u2014 countdown starts after wave 50", count: this.count });
        const removeHint = i18next.t("modifierType:permaRemoveHint", { defaultValue: "Press 10 times to remove" });
        return `${durationLine}\n${removeHint}`;
    }
    ...
    getTooltipRarity(scene: BattleScene): SkillTreeRarity {
        if ((scene as any)?.skillTreeModifierContext === true) return SkillTreeRarity.ROGUE;
        const rank = getPermaModifierRarity(this.permaTypeKey);
        if (rank >= 3) return SkillTreeRarity.LEGENDARY;
...
export class PermaModifierTypeGenerator extends ModifierTypeGenerator {
    ...
    private readonly MIN_COUNT: number = 25;
    ...
    private generateRandomDuration(defaultDuration?: PermaDuration): PermaDuration {
        return PermaDuration.WAVE_BASED;
    }
...
export function getAttackTypeBoosterItemName(type: Type) {
    switch (type) {
        case Type.NORMAL:
            return "Silk Scarf";
        case Type.FIGHTING:
...
export class PlayerPokemonBaseStatBoosterModifierType extends PokemonHeldItemModifierType implements GeneratedPersistentModifierType {
    private localeName: string;
    private stat: Stat;

    constructor(localeName: string, stat: Stat) {
        super("", localeName.replace(/[ \-]/g, "_").toLowerCase(), (_type, args) => new Modifiers.PlayerPokemonBaseStatBoosterModifier(this, (args[0] as Pokemon).id, this.stat));
    ...
    get name(): string {
        return `${i18next.t("modifierType:ModifierType.PlayerPokemonBaseStatBoosterModifierType.name")} ${i18next.t(`modifierType:BaseStatBoosterItem.${this.localeName.replace(/[ \-]/g, "_").toLowerCase()}`)}`;
    }

    getDescription(scene: BattleScene): string {
        return i18next.t("modifierType:ModifierType.PlayerPokemonBaseStatBoosterModifierType.description", {statName: getStatName(this.stat)});
    }
...
export class SkillPointRewardModifierType extends ModifierType {
    public amount: integer;

    constructor(localeKey: string, iconImage: string, amount: integer, group?: string, soundName?: string) {
        super(localeKey, iconImage, (type, _args) => new Modifiers.SkillPointRewardModifier(type, amount), group, soundName);
        this.amount = amount;
    }
...
export class SkillTreeTokenRewardModifierType extends ModifierType {
    ...
    constructor(localeKey: string, iconImage: string, amount: integer, group?: string, soundName?: string, isInverted?: boolean) {
        super(localeKey, iconImage, (type, _args) => new Modifiers.SkillTreeTokenRewardModifier(type, amount), group, soundName, isInverted);
        this.amount = amount;
    }
...
export class EssenceBundleRewardModifierType extends ModifierType {
    public essenceType: Type;
    public amount: integer;

    constructor(localeKey: string, iconImage: string, essenceType: Type, amount: integer, group?: string) {
        super(localeKey, iconImage,
            (type, _args) => new Modifiers.EssenceBundleRewardModifier(type, essenceType, amount),
            group);
    ...
    get name(): string {
        return i18next.t("skillTree:rewards.essenceBundle", {
            amount: this.amount,
            type: i18next.t(`pokemonInfo:Type.${Type[this.essenceType]}`, { defaultValue: Type[this.essenceType] })
        });
    }
...
export class YuTmModifierType extends TmModifierType {
  public readonly targetPokemonId: integer;

  constructor(moveId: Moves, targetPokemonId: integer) {
    super(moveId);
    this.targetPokemonId = targetPokemonId;
    this.setTier(ModifierTier.LUXURY);
    this.selectFilter = (pokemon: PlayerPokemon) => {
      if (pokemon.id !== targetPokemonId) return PartyUiHandler.NoEffectMessage;
  ...
  get name(): string {
    return allMoves[this.moveId].name;
  }
}
...
class AttackTypeBoosterModifierTypeGenerator extends ModifierTypeGenerator {
    constructor() {
        super((party: Pokemon[], pregenArgs?: any[]) => {
            ...
            const attackMoveTypes = party.map(p =>
                p.getMoveset()
                    .map(m => m?.getMove())
                    .filter(m => m instanceof AttackMove && m.type !== Type.UNKNOWN)
                    .map(m => m.type)
            ).flat();
    ...
    constructor(rare: boolean) {
        super((party: Pokemon[], pregenArgs?: any[]) => {
            ...
            const evolutionItemPool = [
                party.filter(p => pokemonEvolutions.hasOwnProperty(p.species.speciesId) && !p.pauseEvolutions && !p.isEvolutionLocked?.()).map(p => {
                ...
                party.filter(p => p.isFusion() && p.fusionSpecies && pokemonEvolutions.hasOwnProperty(p.fusionSpecies.speciesId) && !p.pauseEvolutions && !p.isEvolutionLocked?.()).map(p => {
                ...
                formChangeItemPool = [...new Set(party.filter(p =>
                    ...
                    if (p.species.speciesId === Species.NECROZMA) {
                        ...
                        formChangeItemTriggers.forEach(fc => {
                            ...
                            if (itemTrigger && itemTrigger.active && !p.isEvolutionLocked?.()) {
                        ...
                        formChangeItemTriggers.forEach(fc => {
                            ...
                            if (itemTrigger && itemTrigger.active && !p.scene.findModifier(m =>
                                ...
                                const item = itemTrigger.item;
                                const isSmittyItem = item >= FormChangeItem.SMITTY_AURA && item <= FormChangeItem.SMITTY_VOID;
                                const isGlitchItem = item >= FormChangeItem.GLITCHI_GLITCHI_FRUIT && item <= FormChangeItem.GLITCH_MASTER_PARTS;
                                if (!p.isEvolutionLocked?.() || isSmittyItem || isGlitchItem) {
                                    items.push(item);
                                }
                            }
...
export class TerastallizeModifierType extends PokemonHeldItemModifierType implements GeneratedPersistentModifierType {
    private _teraType: Type;

    constructor(teraType: Type) {
        super("", `${Type[teraType].toLowerCase()}_tera_shard`, (type, args) => new Modifiers.TerastallizeModifier(type as TerastallizeModifierType, (args[0] as Pokemon).id, teraType, undefined, undefined), "tera_shard");

        this._teraType = teraType;
    }
    ...
    get name(): string {
        return i18next.t("modifierType:ModifierType.TerastallizeModifierType.name", {teraType: i18next.t(`pokemonInfo:Type.${Type[this._teraType]}`)});
    }

    getDescription(scene: BattleScene): string {
        return i18next.t("modifierType:ModifierType.TerastallizeModifierType.description", {teraType: i18next.t(`pokemonInfo:Type.${Type[this._teraType]}`)});
    }
...
export class TrainerBondAbilityModifierType extends ModifierType {
  ...
  getDescription(scene: BattleScene): string {
    ...
    const matchingModifiers = scene.findModifiers(m =>
      m instanceof Modifiers.TrainerBondAbilityModifier &&
      m.championId === this.championId &&
      m.ability === this.ability
    );

    const stackCount = matchingModifiers.length > 0 ? matchingModifiers[0].getStackCount() : 0;
    const effectiveChance = stackCount > 0 ? Math.min(1.0, 0.05 * stackCount) : this.bonusChance;
    const chanceText = (effectiveChance * 100).toFixed(0);
  ...
  getPregenArgs(): any[] { return [this.championId, this.ability, this.bonusChance, this.gender]; }
}
  ...
  constructor() {
    super((party: Pokemon[], pregenArgs?: any[]) => {
      ...
      if (pregenArgs && pregenArgs.length >= 2) {
        const [championId, abilityId, activationChance, gender] = pregenArgs as [string, Abilities, number?, PlayerGender?];
        const effectiveGender = gender ?? scene?.gameData?.gender ?? PlayerGender.UNSET;
        return new TrainerBondAbilityModifierType(championId, abilityId, activationChance ?? 0.05, effectiveGender);
      }
      ...
      const gender = scene?.gameData?.gender ?? PlayerGender.UNSET;
      return new TrainerBondAbilityModifierType(championId, abilityId, 0.05, gender);
    });
  }
}
    ...
    super((party: Pokemon[], pregenArgs?: any[]) => {
      ...
      if (pregenArgs && pregenArgs.length > 0 && typeof pregenArgs[0] === 'number') {
        ...
        const selectedSpecies = this.isSignature
          ...
          : SkillTreeSelectors.pickGeneralPokemon(this.championData, scene);
      ...
      let level = scene.currentBattle ? scene.currentBattle.getLvlForWave() : 5;
      level = Math.max(level, 5);

      const currentWave = scene.currentBattle?.waveIndex ?? 1;
      const allowEvolution = currentWave >= 20;

      const adjustedPokemonSpecies = this.isSignature
        ? pokemonSpecies
        : getPokemonSpecies(pokemonSpecies.getSpeciesForLevel(
            level,
            allowEvolution,
            true,
            PartyMemberStrength.STRONGER,
            currentWave,
            scene.gameMode?.isNightmare || false
          ));
      ...
      if (this.isSignature) {
        ...
        if (signatureAltBuildId) {
          ...
          if (altBuild) {
            ...
            const modifier = new Modifiers.PokemonAltBuildModifier(modifierType, newPokemon.id, altBuild);
            modifier.applyAltBuildToPokemon(newPokemon);
          }
        }
        const championTypes = [this.championData.type1, this.championData.type2]
          .filter(t => t !== undefined && t !== null && t !== Type.UNKNOWN) as Type[];
        if (championTypes.length > 0) {
          scene.executeWithSeedOffset(() => {
            assignTypeThemedMoves(scene, newPokemon as any, championTypes, false);
          }, 0, `${scene.seed}_sig_types_${newPokemon.id}`);
          (newPokemon as any)._typeMovePreApplied = true;
        }
      } else {
        const championTypes = [this.championData.type1, this.championData.type2]
          .filter(t => t !== undefined && t !== null && t !== Type.UNKNOWN) as Type[];
        if (championTypes.length > 0) {
          const pokemonTypes = [adjustedPokemonSpecies.type1, adjustedPokemonSpecies.type2]
            .filter(t => t !== undefined && t !== null && t !== Type.UNKNOWN) as Type[];
          const matchingTypes = championTypes.filter(ct => pokemonTypes.includes(ct));
          if (matchingTypes.length > 0) {
            scene.executeWithSeedOffset(() => {
              assignTypeThemedMoves(scene, newPokemon as any, matchingTypes, true);
            }, 0, `${scene.seed}_general_types_${newPokemon.id}`);
            (newPokemon as any)._typeMovePreApplied = true;
          }
        }
      }
...
export class PokemonAltBuildModifierType extends PokemonModifierType {
  ...
  constructor(altBuild: PokemonAltBuildDefinition, targetRank?: number) {
    const localeKey = "modifierType:pokemonAltBuild";
    const iconImage = "pokemon_alt_build";

    const selectFilter = (pokemon: PlayerPokemon): string | null => {
      if (!altBuild.species) {
        return null;
      }

      const speciesMatch = pokemon.species.speciesId === altBuild.species;
      const fusionMatch = pokemon.isFusion() && pokemon.fusionSpecies?.speciesId === altBuild.species;

      if (!speciesMatch && !fusionMatch) {
        const requiredSpecies = getPokemonSpecies(altBuild.species);
        const requiredSpeciesName = requiredSpecies?.name || `Species ${altBuild.species}`;
        return i18next.t("championModifiers:modifierType.pokemonAltBuild.wrongSpecies", { species: requiredSpeciesName });
      }

      if (targetRank && pokemon.altBuildId === altBuild.id) {
        const currentRank = pokemon.altBuildRank || 0;
        if (targetRank <= currentRank) {
          return i18next.t("modifierType:pokemonAltBuild.rankAlreadyUnlocked");
        }
      }
    ...
    super(localeKey, iconImage, (_type, args) =>
        new Modifiers.PokemonAltBuildModifier(this, (args[0] as PlayerPokemon).id, { ...altBuild, rank: targetRank ?? altBuild.rank ?? 1 }), selectFilter, "pokemonAltBuild");

    this.altBuild = altBuild;
    this.targetRank = targetRank ?? altBuild.rank ?? 1;
  }

  get name(): string {
    const buildName = ChampionUtils.getAltBuildDisplayName(this.altBuild.id);
    const rankSuffix = Utils.intToRoman(this.targetRank);
    return i18next.t("championModifiers:modifierType.pokemonAltBuild.name", { buildName: `${buildName} ${rankSuffix}` });
  }

  getDescription(scene: BattleScene): string {
    const buildName = ChampionUtils.getAltBuildDisplayName(this.altBuild.id);
    const description = ChampionUtils.getAltBuildDisplayDescription(this.altBuild.id, scene);
    return i18next.t("championModifiers:modifierType.pokemonAltBuild.description", {
      buildName,
      description
    });
  }
  ...
  constructor() {
    super((party: Pokemon[], pregenArgs?: any[]) => {
      if (pregenArgs && pregenArgs.length > 0) {
        const [altBuildId, targetRank] = pregenArgs as [PokemonAltBuildId, number?];
        const altBuild = POKEMON_ALT_BUILDS[altBuildId];
        if (!altBuild) return null;
        return new PokemonAltBuildModifierType(altBuild, targetRank);
      }
      const scene = party?.[0]?.scene as BattleScene;
      ...
      const altBuild = POKEMON_ALT_BUILDS[altBuildId];
      if (!altBuild) return null;
      return new PokemonAltBuildModifierType(altBuild, 1);
    });
  }
}
...
export class TeraAbilityModifierType extends ModifierType implements GeneratedPersistentModifierType {
  ...
  constructor(championId: string, abilityId: Abilities, teraType: Type, activationChance: number = 0.10) {
    super("championModifiers:modifierType.teraAbility", "stellar_tera_shard", (type: ModifierType) => new Modifiers.TeraAbilityModifier(type, championId, abilityId, teraType, activationChance), "teraAbility", undefined, true);
    this.championId = championId;
  ...
  get name(): string {
    const abilityName = allAbilities[this.abilityId].name;
    const typeName = i18next.t(`pokemonInfo:Type.${Type[this.teraType]}`, { defaultValue: Utils.toReadableString(Type[this.teraType]) });
    return i18next.t("championModifiers:modifierType.teraAbility.name", { ability: abilityName, type: typeName });
  }

  getDescription(scene: BattleScene): string {
    const abilityName = allAbilities[this.abilityId].name;
    const abilityDescription = allAbilities[this.abilityId].description;
    const typeName = i18next.t(`pokemonInfo:Type.${Type[this.teraType]}`, { defaultValue: Utils.toReadableString(Type[this.teraType]) });

    const matchingModifiers = scene.findModifiers(m =>
      m instanceof Modifiers.TeraAbilityModifier &&
      m.championId === this.championId &&
      m.abilityId === this.abilityId
    );

    const stackCount = matchingModifiers.length > 0 ? matchingModifiers[0].getStackCount() : 0;
    const effectiveChance = stackCount > 0 ? Math.min(1.0, 0.10 * stackCount) : this.activationChance;
    const chanceText = (effectiveChance * 100).toFixed(0);

    return i18next.t("championModifiers:modifierType.teraAbility.description", {
      ability: abilityName,
      abilityDescription: abilityDescription,
      type: typeName,
      chance: chanceText
  ...
  getPregenArgs(): any[] { return [this.championId, this.abilityId, this.teraType, this.activationChance]; }
}
...
export class TeraAbilityModifierTypeGenerator extends ModifierTypeGenerator {
  constructor() {
    super((party: Pokemon[], pregenArgs?: any[]) => {
      if (pregenArgs && pregenArgs.length >= 3) {
        const [championId, abilityId, teraType, activationChance] = pregenArgs as [string, Abilities, Type, number?];
        return new TeraAbilityModifierType(championId, abilityId, teraType, activationChance ?? 0.10);
      }
      const scene = party?.[0]?.scene as BattleScene;
      const champData = scene?.gameData?.activeSkillTree?.championData as PlayableChampionData | undefined;
      const championId = scene?.gameData?.selectedChampionId;
      if (!championId || !champData) return null;
      const result = SkillTreeSelectors.pickTeraAbilityWithType(champData);
      if (result == null) return null;
      return new TeraAbilityModifierType(championId, result.ability, result.type, 0.10);
    });
  }
}
...
export class ChampionPokemonStatBoosterModifierType extends PokemonModifierType {
  ...
  constructor(championId: string, stats: Stat[], boostPercent: number = 0.03, championTypes?: Type[]) {
    const localeKey = "modifierType:championPokemonStatBooster";
    const iconImage = "protein";

    super(localeKey, iconImage, (_type, args) =>
        new Modifiers.ChampionPokemonStatBoosterModifier(this, (args[0] as Pokemon).id, championId, stats, boostPercent, championTypes), "champion");
  ...
  getDescription(scene: BattleScene): string {
    const championName = ChampionUtils.getChampionDisplayName(this.championId);
    const statNames = this.stats.map(stat => getStatName(stat)).join(", ");
    const firstStatName = this.stats.length > 0 ? getStatName(this.stats[0]) : "";
    const percentText = (this.boostPercent * 100).toFixed(0);
    const typeNames = this.championTypes.map(t => i18next.t(`pokemonInfo:Type.${Type[t]}`)).join("/");

    if (this.championTypes.length > 0 && this.stats.length > 1) {
      return i18next.t("championModifiers:modifierType.championPokemonStatBooster.descriptionConditional", {
        champion: championName,
        stats: statNames,
        types: typeNames,
        percent: percentText,
  ...
  getPregenArgs(): any[] { return [this.championId, this.stats, this.boostPercent, this.championTypes]; }
}
...
export class ChampionPokemonStatBoosterModifierTypeGenerator extends ModifierTypeGenerator {
  constructor() {
    super((party: Pokemon[], pregenArgs?: any[]) => {
      if (pregenArgs && pregenArgs.length >= 2) {
        const [championId, stats, boostPercent, championTypes] = pregenArgs as [string, Stat[], number?, Type[]?];
        return new ChampionPokemonStatBoosterModifierType(championId, stats, boostPercent ?? 0.03, championTypes);
      }
      const scene = party?.[0]?.scene as BattleScene;
      const championId = (scene as any)?.gameData?.selectedChampionId as string | undefined;
      const champData = (scene as any)?.gameData?.activeSkillTree?.championData as PlayableChampionData | undefined;
...
export const modifierTypes = {
    ...
    FORBIDDEN_FORM_UNLOCK: () => new ModifierTypeGenerator((party: Pokemon[], pregenArgs?: any[]) => {
        ...
        if (!gd) return null;
        const candidates: ForbiddenFormUnlockCandidate[] = [];
        const seen = new Set<string>();
        const questIds = Object.values(QuestUnlockables).filter(v => typeof v === "number");
        for (const questId of questIds) {
            try {
                const questUnlockData = gd.getQuestUnlockDataFromModifierTypes(questId as any);
                const rt = questUnlockData?.rewardType;
                const rid = questUnlockData?.rewardId;
                const ids = Array.isArray(rid) ? rid : [rid];
                if (!ids.some((v: any) => typeof v === "number")) continue;
                const candidate: ForbiddenFormUnlockCandidate = { kind: "QUEST_FORM", questUnlockData };
                const key = `QF_${rt}_${ids[0]}`;
                if (seen.has(key)) continue;
                seen.add(key);
                if (rt === RewardType.GLITCH_FORM_A || rt === RewardType.GLITCH_FORM_B ||
                    rt === RewardType.GLITCH_FORM_C || rt === RewardType.GLITCH_FORM_D ||
                    rt === RewardType.GLITCH_FORM_E || rt === RewardType.SMITTY_FORM ||
                    rt === RewardType.SMITTY_FORM_B) {
        ...
        for (const [systemName, data] of Object.entries(modGlitchFormData || {})) {
            ...
            const key = `MF_${systemName}`;
            if (seen.has(key)) continue;
            seen.add(key);
            candidates.push({ kind: "MOD_FORM", systemName, speciesId: d.speciesId, formName: d.formName } as any);
        }
        for (const f of universalSmittyForms || []) {
            const formName = (f as any)?.formName;
            if (!formName) continue;
            const key = `US_${formName}`;
            if (seen.has(key)) continue;
            seen.add(key);
            candidates.push({ kind: "UNI_SMITTY", formName } as any);
        }
        const isUnlocked = (c: ForbiddenFormUnlockCandidate): boolean => {
            try {
                if (c.kind === "MOD_FORM") return gd.isModFormUnlocked((c as any).systemName);
                if (c.kind === "UNI_SMITTY") return gd.isUniSmittyFormUnlocked((c as any).formName);
                if (c.kind === "QUEST_FORM") {
                    const q = (c as any).questUnlockData;
                    const ids = Array.isArray(q?.rewardId) ? q.rewardId : [q?.rewardId];
                    return ids.filter((v: any) => typeof v === "number").some((id: any) => gd.canUseGlitchOrSmittyForm(id, q?.rewardType));
                }
            } catch {}
            return true;
        };
    ...
    COLLECTED_TYPE: () => new ModifierTypeGenerator((party: Pokemon[], pregenArgs?: any[]) => {
        ...
        const scene = party?.[0]?.scene as any;
        let selected: Type = Type.NORMAL;

        try {
            const runtimeWeights = scene?.gameData?.activeSkillTree?.essenceTypeWeights as Partial<Record<Type, number>> | undefined;
            const allowList = undefined as unknown as Type[] | undefined;
            const basePool = (Utils.getEnumValues(Type).filter((t: number) => t >= Type.NORMAL && t <= Type.FAIRY) as unknown) as Type[];
            const pool = Array.isArray(allowList) && allowList.length > 0 ? basePool.filter(t => allowList.includes(t)) : basePool;
            const weighted: Array<{ t: Type; w: number }> = pool.map(t => ({ t, w: Math.max(0, runtimeWeights?.[t] ?? 1) }));
            const totalW = weighted.reduce((s, r) => s + r.w, 0);
            if (totalW > 0) {
                let roll = Utils.randSeedInt(Math.ceil(totalW * 100)) / 100;
                for (const { t, w } of weighted) {
                    const slice = w / totalW;
                    if ((roll -= slice) <= 0) { selected = t; break; }
                }
            }
        } catch {
        ...
        return new CollectedTypeModifierType(selected);
    }),

    PERMA_COLLECTED_TYPE: () => new PermaCollectedTypeModifierType(),

    MOVE_UPGRADE: () => new MoveUpgradeModifierTypeGenerator(),
    LOW_TIER_MOVE_UPGRADE: () => new MoveUpgradeModifierTypeGenerator(true),
    ...
    TRAINER_BOND_ABILITY: () => new TrainerBondAbilityModifierTypeGenerator(),
    TERA_ABILITY: () => new TeraAbilityModifierTypeGenerator(),
    POKEMON_ALT_BUILD: () => new PokemonAltBuildModifierTypeGenerator(),
    ...
    PLAYER_BASE_STAT_BOOSTER: () => new ModifierTypeGenerator((party: Pokemon[], pregenArgs?: any[]) => {
        if (pregenArgs && (pregenArgs.length === 1) && (pregenArgs[0] in Stat)) {
            const stat = pregenArgs[0] as Stat;
            return new PlayerPokemonBaseStatBoosterModifierType(getBaseStatBoosterItemName(stat), stat);
        }
        const randStat = Utils.randSeedInt(6) as Stat;
        return new PlayerPokemonBaseStatBoosterModifierType(getBaseStatBoosterItemName(randStat), randStat);
    }),
    ...
    TYPE_BALL_RANDOM: () => new TypeBallModifierTypeGenerator("random"),
...
function hasMaximumBalls(party: Pokemon[], ballType: PokeballType): boolean {
    ...
    return (scene.gameMode.isClassic && scene.pokeballCounts[ballType] >= MAX_PER_TYPE_POKEBALLS);
}

function shouldHideHealingModifier(party: Pokemon[]): boolean {
    const scene = party[0]?.scene;
    if (!scene?.currentBattle) return false;
    if (scene.currentBattle.waveIndex % 10 === 0) {
...
const modifierPool: ModifierPool = {
    [ModifierTier.COMMON]: [
        ...
        new WeightedModifierType(modifierTypes.ANY_ABILITY, (party: Pokemon[]) => glitchPieceWeightValAdjustment(party, 12), 12),
        new WeightedModifierType(modifierTypes.ANY_ABILITY, glitchPieceWeightAdjustment, 6),
        new WeightedModifierType(modifierTypes.ANY_PASSIVE_ABILITY, (party: Pokemon[]) => glitchPieceWeightValAdjustment(party, 12), 12),
    ...
    [ModifierTier.GREAT]: [
        ...
        new WeightedModifierType(modifierTypes.ANYTM_ULTRA, glitchPieceWeightAdjustment, 6),
        new WeightedModifierType(modifierTypes.ABILITY_SWITCHER, (party: Pokemon[]) => glitchPieceWeightValAdjustment(party, 10), 10),
        new WeightedModifierType(modifierTypes.ANY_ABILITY, (party: Pokemon[]) => glitchPieceWeightValAdjustment(party, 12), 12),
        new WeightedModifierType(modifierTypes.ANY_ABILITY, glitchPieceWeightAdjustment, 6),
        new WeightedModifierType(modifierTypes.ANY_PASSIVE_ABILITY, (party: Pokemon[]) => glitchPieceWeightValAdjustment(party, 12), 12),
        new WeightedModifierType(modifierTypes.ANY_PASSIVE_ABILITY, glitchPieceWeightAdjustment, 6),
        ...
        new WeightedModifierType(modifierTypes.COLLECTED_TYPE, glitchPieceWeightAdjustment, 5),
    ...
    [ModifierTier.ULTRA]: [
        ...
        new WeightedModifierType(modifierTypes.ANY_ABILITY, (party: Pokemon[]) => glitchPieceWeightValAdjustment(party, 14), 14),
        new WeightedModifierType(modifierTypes.ANY_PASSIVE_ABILITY, (party: Pokemon[]) => glitchPieceWeightValAdjustment(party, 14), 14),
        ...
        new WeightedModifierType(modifierTypes.ADD_POKEMON, glitchPiecePermaMidWeightAdjustment ? 14 : 0, 14),
    ...
    [ModifierTier.MASTER]: [
        ...
        new WeightedModifierType(modifierTypes.SELECTABLE_PMONEY_4OR5, glitchPiecePermaMoneyWeightAdjustment ? 8 : 0, 8),
        new WeightedModifierType(modifierTypes.MEGA_BRACELET, (party: Pokemon[]) => {
            if (party[0].scene.getModifiers(Modifiers.MegaEvolutionAccessModifier).length > 0) {
                return 0;
            }
            return Math.min(Math.ceil(party[0].scene.currentBattle.waveIndex / 20), 4) * 3;
        }, 1),
        new WeightedModifierType(modifierTypes.DYNAMAX_BAND, (party: Pokemon[]) => {
            if (party[0].scene.getModifiers(Modifiers.GigantamaxAccessModifier).length > 0) {
                return 0;
            }
            return Math.min(Math.ceil(party[0].scene.currentBattle.waveIndex / 20), 4) * 3;
        }, 1),
        new WeightedModifierType(modifierTypes.MASTER_BALL, (party: Pokemon[]) => (hasMaximumBalls(party, PokeballType.MASTER_BALL) || party[0].scene.pokeballCounts[PokeballType.MASTER_BALL] >= 5) ? 0 : 4, 4),
        new WeightedModifierType(modifierTypes.SHINY_CHARM, 8),
        new WeightedModifierType(modifierTypes.HEALING_CHARM, 8),
...
const collectorModifierPool: ModifierPool = {
    [ModifierTier.COMMON]: [
        new WeightedModifierType(modifierTypes.ANYTM_ULTRA, 3),
        new WeightedModifierType(modifierTypes.GLITCH_PIECE, 3),
    ...
    [ModifierTier.GREAT]: [
        ...
        new WeightedModifierType(modifierTypes.ANY_ABILITY, 12),
        new WeightedModifierType(modifierTypes.ANY_PASSIVE_ABILITY, 12),
        new WeightedModifierType(modifierTypes.BIG_NUGGET, skipInLastClassicWaveOrDefault(5)),
        new WeightedModifierType(modifierTypes.TERA_SHARD, 1),
        new WeightedModifierType(modifierTypes.VOUCHER, (party: Pokemon[], rerollCount: integer) => !party[0].scene.gameMode.isDaily ? Math.max(1 - rerollCount, 0) : 0, 1),
        new WeightedModifierType(modifierTypes.SELECTABLE_PMONEY_1, glitchPiecePermaMoneyWeightAdjustment, 4),
    ...
    [ModifierTier.ULTRA]: [
        ...
        new WeightedModifierType(modifierTypes.ADD_POKEMON, 3),
        new WeightedModifierType(modifierTypes.ANY_PASSIVE_ABILITY, 14),
        new WeightedModifierType(modifierTypes.ABILITY_SACRIFICE, 2),
        new WeightedModifierType(modifierTypes.PASSIVE_ABILITY_SACRIFICE, 3),
        new WeightedModifierType(modifierTypes.MOVE_SACRIFICE, 6),
        new WeightedModifierType(modifierTypes.BIG_NUGGET, 12),
        ...
        new WeightedModifierType(modifierTypes.TERA_ORB, 2),
        new WeightedModifierType(modifierTypes.VOUCHER, 2),
        new WeightedModifierType(modifierTypes.SELECTABLE_PMONEY_2, 4),
    ...
    [ModifierTier.ROGUE]: [
        new WeightedModifierType(modifierTypes.MOVE_UPGRADE, 12),
        new WeightedModifierType(modifierTypes.SELECTABLE_PMONEY_3, 4),
        new WeightedModifierType(modifierTypes.RELIC_GOLD, 4),
        new WeightedModifierType(modifierTypes.FORM_CHANGE_ITEM, 8),
        new WeightedModifierType(modifierTypes.VOUCHER_PLUS, 2),
        new WeightedModifierType(modifierTypes.RARER_CANDY, 1),
        new WeightedModifierType(modifierTypes.DNA_SPLICERS, 1),
        new WeightedModifierType(modifierTypes.SKILL_POINTS, 2),
        new WeightedModifierType(modifierTypes.MASTER_BALL, (party: Pokemon[]) => (hasMaximumBalls(party, PokeballType.MASTER_BALL) || party[0].scene.pokeballCounts[PokeballType.MASTER_BALL] >= 5) ? 0 : 4, 4),
    ].map(m => {
        m.setTier(ModifierTier.ROGUE);
        return m;
    ...
    [ModifierTier.MASTER]: [
        new WeightedModifierType(modifierTypes.BASE_STAT_BOOSTER, 2),
        new WeightedModifierType(modifierTypes.SKILL_TOKENS, 2),
        new WeightedModifierType(modifierTypes.ANY_SMITTY_PASSIVE_ABILITY, glitchPieceWeightAdjustment, 1),
        new WeightedModifierType(modifierTypes.ANY_SMITTY_ABILITY, glitchPieceWeightAdjustment, 1),
        new WeightedModifierType(modifierTypes.SELECTABLE_PMONEY_4OR5, glitchPiecePermaMoneyWeightAdjustment ? 8 : 0, 8),
        new WeightedModifierType(modifierTypes.MEGA_BRACELET, (party: Pokemon[]) => {
...
const wildModifierPool: ModifierPool = {
    ...
    [ModifierTier.ULTRA]: [
        ...
        new WeightedModifierType(modifierTypes.MOVE_UPGRADE, 10),
    ...
    [ModifierTier.MASTER]: [
        ...
        new WeightedModifierType(modifierTypes.TYPE_SWITCHER, (party: Pokemon[]) => party[0].scene.currentBattle.waveIndex == 500 ? 0 : 1,  1),
        new WeightedModifierType(modifierTypes.STAT_SWITCHER, 2),
        new WeightedModifierType(modifierTypes.STAT_SACRIFICE, 1),
...
const trainerModifierPool: ModifierPool = {
    [ModifierTier.COMMON]: [
        ...
        new WeightedModifierType(modifierTypes.MOVE_UPGRADE, 8),
    ].map(m => {
        m.setTier(ModifierTier.COMMON);
        return m;
    }),
...
const testModPool: ModifierPool = {
    [ModifierTier.COMMON]: [
...
const debugPool: ModifierPool = {
    ...
    [ModifierTier.GREAT]: [
        ...
        new WeightedModifierType(modifierTypes.MEGA_BRACELET, 12),
        new WeightedModifierType(modifierTypes.MEGA_BRACELET, 12),
        new WeightedModifierType(modifierTypes.ADD_POKEMON, 12),
        new WeightedModifierType(modifierTypes.ADD_POKEMON, 12),
        new WeightedModifierType(modifierTypes.ADD_POKEMON, 12),
        new WeightedModifierType(modifierTypes.ADD_POKEMON, 12),
        new WeightedModifierType(modifierTypes.ADD_POKEMON, 12),
    ...
    [ModifierTier.ROGUE]: [
        new WeightedModifierType(modifierTypes.SKILL_POINTS, 12),
        new WeightedModifierType(modifierTypes.SKILL_POINTS, 12),
        new WeightedModifierType(modifierTypes.SKILL_POINTS, 12),
        new WeightedModifierType(modifierTypes.SKILL_POINTS, 12),
        new WeightedModifierType(modifierTypes.SKILL_POINTS, 12),
...
export function getModifierType(modifierTypeFunc: ModifierTypeFunc): ModifierType {
    ...
    if (modifierType instanceof ModifierTypeGenerator) {
        modifierType = modifierType.generateType([]);
    }
    if (!modifierType.id) {
...
let collectorModifierPoolThresholds = {};
let collectorIgnoredPoolIndexes = {};
...
export function getModifierPoolForType(poolType: ModifierPoolType, isTestMod: boolean = false): ModifierPool {
    ...
    switch (poolType) {
        ...
        case ModifierPoolType.DRAFT:
            pool = debugDraftPool;
            break;
        case ModifierPoolType.COLLECTOR:
            pool = collectorModifierPool;
            break;
    }
    return isTestMod ? testModPool : pool;

}

export function getTierFromModifierID(modifierID: string): ModifierTier {
    const pool = getModifierPoolForType(ModifierPoolType.PLAYER);

    for (const [tierKey, weightedModifiers] of Object.entries(pool)) {
        const tier = parseInt(tierKey) as ModifierTier;
        const found = weightedModifiers.find(wm => wm.modifierType.id === modifierID);
        if (found) {
            return tier;
        }
    }
...
export function regenerateModifierPoolThresholds(party: Pokemon[], poolType: ModifierPoolType, rerollCount: integer = 0) {
    ...
    const pool = getModifierPoolForType(poolType, scene.gameMode.isTestMod);
    ...
    const thresholds = Object.fromEntries(new Map(Object.keys(pool).map(t => {
        ...
        pool[t].reduce((total: integer, modifierType: WeightedModifierType) => {
            ...
            const existingModifiers = scene.findModifiers(m => m.type.id === weightedModifierType.modifierType.id, poolType === ModifierPoolType.PLAYER);
            ...
            const allowRepeatDespiteExisting =
              weightedModifierType.modifierType.id === "ANY_ABILITY" ||
              weightedModifierType.modifierType.id === "ANY_PASSIVE_ABILITY";

            const weight = !existingModifiers.length
            || allowRepeatDespiteExisting
            || itemModifierType instanceof PokemonHeldItemModifierType
            || itemModifierType instanceof FormChangeItemModifierType
            || existingModifiers.find(m => m.stackCount < m.getMaxStackCount(scene, true))
    ...
    switch (poolType) {
        ...
        case ModifierPoolType.DRAFT:
            ...
            break;
        case ModifierPoolType.COLLECTOR:
            collectorModifierPoolThresholds = thresholds;
            collectorIgnoredPoolIndexes = ignoredIndexes;
            break;
    }
}
...
export function getPlayerModifierTypeOptions(count: integer, party: PlayerPokemon[], modifierTiers?: ModifierTier[], isDraftMode: boolean = false, pathNodeFilter?: PathNodeTypeFilter, poolType?: ModifierPoolType): ModifierTypeOption[] {
    const options: ModifierTypeOption[] = [];
    const duplicateRetryCount = Math.min(count * 5, 50);
    const nullRetryCount = 100;

    const modifierPoolType = poolType ? poolType : isDraftMode ? ModifierPoolType.DRAFT : ModifierPoolType.PLAYER

    new Array(count).fill(0).map((_, i) => {
        let candidate: ModifierTypeOption | null = null;
        let nullRetries = 0;

        while (!candidate && nullRetries < nullRetryCount) {
            candidate = getNewModifierTypeOption(party, modifierPoolType, modifierTiers?.length > i ? modifierTiers[i] : undefined, undefined, 0, pathNodeFilter);
            if (!candidate) {
                nullRetries++;
            }
        }
        ...
        if (!candidate) {
            const rareCandyType = modifierTypes.RARE_CANDY();
            if (!rareCandyType.id) {
                rareCandyType.withIdFromFunc(modifierTypes.RARE_CANDY);
            }
            candidate = new ModifierTypeOption(rareCandyType, 0);
        }

        let r = 0;
        const isMoveUpgrade = candidate && candidate.type.name && candidate.type.name.includes("Upgrade");
        while (candidate && options.length && ++r < duplicateRetryCount && !isMoveUpgrade && options.filter(o => o.type.name === candidate.type.name || o.type.group === candidate.type.group).length) {
...
export function overridePlayerModifierTypeOptions(options: ModifierTypeOption[], party: PlayerPokemon[]) {
    if (!Overrides.ITEM_REWARD_OVERRIDE.length || !options.length) return;
    const shuffled = Utils.randSeedShuffle([...Overrides.ITEM_REWARD_OVERRIDE]);
    const pickCount = Math.min(options.length, shuffled.length);
...
export function getPlayerShopModifierTypeOptionsForWave(scene: BattleScene, baseCost: integer): ModifierTypeOption[] | null {
    if (!scene.currentBattle) return null;
    const waveIndex = scene.gameMode.isNightmare ? scene.currentBattle.waveIndex % 100 : scene.currentBattle.waveIndex;
    const forceAll = Overrides.DEBUG_SHOP_FORCE_ALL_ITEMS;
    if (forceAll || Utils.randSeedInt(100) <= 1) {
        baseCost = 0;
    }
    ...
    const hasShop = scene.gameMode.hasShopCheck(scene) && (scene.gameMode.isChaosMode || waveIndex % 10 != 0);

    const isStarterQuestCompleted = forceAll || scene.gameData.checkQuestState(QuestUnlockables.STARTER_CATCH_QUEST, QuestState.COMPLETED);
    const isNuzlightQuestCompleted = forceAll || scene.gameData.checkQuestState(QuestUnlockables.NUZLIGHT_UNLOCK_QUEST, QuestState.COMPLETED);
    ...
    const options = [
        [
            ...
            ...((forceAll || Utils.randSeedInt(100) < 25) ? [createOptionForGenerator(modifierTypes.BERRY, 0, baseCost)] : []),
            ...((forceAll || Utils.randSeedInt(100) < 25) ? [createOptionForGenerator(modifierTypes.MINT, 0, baseCost)] : []),
            ...((forceAll || (Utils.randSeedInt(100) < 10 && isNuzlightQuestCompleted && (forceAll || scene.releaseItemsEnabledForRun))) ? [createOptionForGenerator(modifierTypes.TYPE_SACRIFICE, 0, baseCost)] : []),
            ...((forceAll || (Utils.randSeedInt(100) <= 1 && isNuzlightQuestCompleted && (forceAll || scene.releaseItemsEnabledForRun))) ? [createOptionForGenerator(modifierTypes.STAT_SACRIFICE, 0, baseCost *3)] : []),
            ...((forceAll || (Utils.randSeedInt(100) < 10 && isNuzlightQuestCompleted)) ? [createOptionForGenerator(modifierTypes.ADD_POKEMON, 0, baseCost)] : []),
            ...((forceAll || Utils.randSeedInt(100) < 10) ? [createOptionForGenerator(modifierTypes.TYPE_SWITCHER, 0, baseCost)] : []),
            ...((forceAll || Utils.randSeedInt(100) < 10) ? [createOptionForGenerator(modifierTypes.ABILITY_SWITCHER, 0, baseCost)] : []),
            ...((forceAll || (Utils.randSeedInt(100) <= 2 && isNuzlightQuestCompleted)) ? [createOptionForGenerator(modifierTypes.PRIMARY_TYPE_SWITCHER, 0, baseCost * 2)] : []),
            ...((forceAll || (Utils.randSeedInt(100) <= 2 && isNuzlightQuestCompleted)) ? [createOptionForGenerator(modifierTypes.SECONDARY_TYPE_SWITCHER, 0, baseCost * 2)] : []),
            ...((forceAll || (Utils.randSeedInt(100) < 5 && isNuzlightQuestCompleted && (forceAll || scene.releaseItemsEnabledForRun))) ? [createOptionForGenerator(modifierTypes.ABILITY_SACRIFICE, 0, baseCost)] : []),
            ...((forceAll || (Utils.randSeedInt(100) < 5 && isNuzlightQuestCompleted)) ? [createOptionForGenerator(modifierTypes.MOVE_UPGRADE, 0, baseCost * 5)] : []),
            ...((forceAll || Utils.randSeedInt(100) < 5) ? [createOptionForGenerator(modifierTypes.TM_GREAT, 0, baseCost * 2)] : []),
            ...((forceAll || Utils.randSeedInt(100) < 10) ? [createOptionForGenerator(modifierTypes.TM_COMMON, 0, baseCost)] : []),
            ...((forceAll || (Utils.randSeedInt(100) <= 1 && isNuzlightQuestCompleted)) ? [createOptionForGenerator(modifierTypes.TM_ULTRA, 0, baseCost * 3)] : []),
            ...((forceAll || (Utils.randSeedInt(100) < 5 && isNuzlightQuestCompleted)) ? [createOptionForGenerator(modifierTypes.ANYTM_GREAT, 0, baseCost * 2)] : []),
            ...((forceAll || (Utils.randSeedInt(100) < 10 && isNuzlightQuestCompleted)) ? [createOptionForGenerator(modifierTypes.ANYTM_COMMON, 0, baseCost)] : []),
            ...((forceAll || (Utils.randSeedInt(100) <= 1 && isNuzlightQuestCompleted)) ? [createOptionForGenerator(modifierTypes.ANYTM_ULTRA, 0, baseCost * 3)] : []),
            ...((forceAll || Utils.randSeedInt(100) <= 2) ? [createOptionForGenerator(modifierTypes.ANY_ABILITY, 0, baseCost * 3)] : []),
            ...((forceAll || (Utils.randSeedInt(100) <= 5 && isNuzlightQuestCompleted)) ? [createOptionForGenerator(modifierTypes.COLLECTED_TYPE, 0, baseCost)] : []),
            ...((forceAll || (Utils.randSeedInt(100) <= 1 && isNuzlightQuestCompleted)) ? [createOptionForGenerator(modifierTypes.ANY_PASSIVE_ABILITY, 0, baseCost * 3)] : []),
            ...((forceAll || Utils.randSeedInt(100) < 5) ? [new ModifierTypeOption(modifierTypes.BIG_NUGGET(), 0, 0)] : []),
            ...((forceAll || Utils.randSeedInt(100) < 3) ? [new ModifierTypeOption(modifierTypes.GLITCH_PIECE(), 0, 0)] : []),
        ],
        [
            ...(hasShop ? [
            ...
            ...((forceAll || Utils.randSeedInt(100) < 15) ?
                ...
                : []),
            ...((forceAll || Utils.randSeedInt(100) < 20) ? [new ModifierTypeOption(modifierTypes.SELECTABLE_PMONEY_2(), 0, baseCost * 3)] : []),
            ...((forceAll || Utils.randSeedInt(500) < 5) ? [new ModifierTypeOption(modifierTypes.RELIC_GOLD(), 0, 0)] : []),
            ...((forceAll || (Utils.randSeedInt(1000) < 1 && (() => {
                const glitchModifier = scene.findModifier(m => m instanceof Modifiers.GlitchPieceModifier) as Modifiers.GlitchPieceModifier;
            ...
            ...((forceAll || Utils.randSeedInt(100) <= 1) ? [new ModifierTypeOption(modifierTypes.RARER_CANDY(), 0, baseCost * 6)] : []),
             ...((forceAll || (Utils.randSeedInt(100) <= 10 && isNuzlightQuestCompleted && (!scene.gameMode.hasShopCheck(scene) || scene.gameMode.isNuzlockeActive(scene)) && scene.getParty().length < 6)) ? [createOptionForGenerator(modifierTypes.DRAFT_POKEMON, 0, baseCost * 3)] : []),
        ...
        [
           ...
           ...(isStarterQuestCompleted ? [
                ...((forceAll || Utils.randSeedInt(250) < 3) ? [new ModifierTypeOption(modifierTypes.DYNAMAX_BAND(), 0, baseCost * 8)] : []),
                ...((forceAll || Utils.randSeedInt(250) < 3) ? [new ModifierTypeOption(modifierTypes.MEGA_BRACELET(), 0, baseCost * 8)] : []),
                ...((forceAll || Utils.randSeedInt(250) < 3) ? [new ModifierTypeOption(modifierTypes.TERA_ORB(), 0, baseCost * 8)] : [])
            ] : []),
            ...((forceAll || Utils.randSeedInt(100) < 15) ? [new ModifierTypeOption(modifierTypes.SELECTABLE_PMONEY_3(), 0, baseCost * 4)] : []),
        ],
        [
            ...(hasShop ? [
            ...
            ...((forceAll || Utils.randSeedInt(100) < 10) ? [new ModifierTypeOption(modifierTypes.SELECTABLE_PMONEY_4(), 0, baseCost * 10)] : []),
        ]
    ];
...
function getPathNodeModifiers(pathNodeFilter: PathNodeTypeFilter, _tier: ModifierTier, party: Pokemon[]): WeightedModifierType[] {
    ...
    switch (pathNodeFilter) {
        ...
        case PathNodeTypeFilter.GREAT_BALL_ITEMS:
            if (pool[ModifierTier.GREAT]) {
                return pool[ModifierTier.GREAT].slice();
            }
            break;

        case PathNodeTypeFilter.ULTRA_BALL_ITEMS:
            if (pool[ModifierTier.ULTRA]) {
                return pool[ModifierTier.ULTRA].slice();
            }
            break;
        ...
        case PathNodeTypeFilter.ANY_TMS:
            ...
            break;
        case PathNodeTypeFilter.TERA_SHARDS:
            modifiers.push(new WeightedModifierType(modifierTypes.TERA_SHARD, 5));
            break;
        ...
        case PathNodeTypeFilter.COLLECTED_SHOP:
            break;
        ...
        case PathNodeTypeFilter.HEAL_ITEMS:
            modifiers.push(new WeightedModifierType(modifierTypes.REVIVE, 3));
            modifiers.push(new WeightedModifierType(modifierTypes.ELIXIR, 3));
            modifiers.push(new WeightedModifierType(modifierTypes.SUPER_POTION, 3));
            modifiers.push(new WeightedModifierType(modifierTypes.HYPER_POTION, 4));
            modifiers.push(new WeightedModifierType(modifierTypes.MAX_POTION, 2));
            modifiers.push(new WeightedModifierType(modifierTypes.FULL_HEAL, 3));
            if (tier === ModifierTier.ROGUE) {
                modifiers.push(new WeightedModifierType(modifierTypes.FULL_RESTORE, 3));
                modifiers.push(new WeightedModifierType(modifierTypes.MAX_REVIVE, 2));
                modifiers.push(new WeightedModifierType(modifierTypes.MAX_ELIXIR, 3));
            }
            else if (tier === ModifierTier.MASTER) {
                modifiers.push(new WeightedModifierType(modifierTypes.REVIVER_SEED, 1));
                modifiers.push(new WeightedModifierType(modifierTypes.SACRED_ASH, 1));
            }
            break;

        case PathNodeTypeFilter.REVIVER_SEED:
            modifiers.push(new WeightedModifierType(modifierTypes.REVIVER_SEED, 1));
            break;

        case PathNodeTypeFilter.SACRED_ASH:
            modifiers.push(new WeightedModifierType(modifierTypes.SACRED_ASH, 1));
            break;

        case PathNodeTypeFilter.SHELL_BELL:
            modifiers.push(new WeightedModifierType(modifierTypes.SHELL_BELL, 1));
            break;

        case PathNodeTypeFilter.LEFTOVERS:
            modifiers.push(new WeightedModifierType(modifierTypes.LEFTOVERS, 1));
            break;

        case PathNodeTypeFilter.QUICK_CLAW:
            modifiers.push(new WeightedModifierType(modifierTypes.QUICK_CLAW, 1));
            break;

        case PathNodeTypeFilter.WIDE_LENS:
            modifiers.push(new WeightedModifierType(modifierTypes.WIDE_LENS, 1));
            break;

        case PathNodeTypeFilter.GRIP_CLAW:
            modifiers.push(new WeightedModifierType(modifierTypes.GRIP_CLAW, 1));
            break;

        case PathNodeTypeFilter.EVIOLITE:
            modifiers.push(new WeightedModifierType(modifierTypes.EVIOLITE, 1));
            break;

        case PathNodeTypeFilter.SCOPE_LENS:
            modifiers.push(new WeightedModifierType(modifierTypes.SCOPE_LENS, 1));
            break;

        case PathNodeTypeFilter.VITAMIN:
            modifiers.push(new WeightedModifierType(modifierTypes.PLAYER_BASE_STAT_BOOSTER, 1));
            break;

        case PathNodeTypeFilter.MOVE_UPGRADE:
            modifiers.push(new WeightedModifierType(modifierTypes.MOVE_UPGRADE, 1));
            break;
...
function getRandomTmFallbackForTier(tier: ModifierTier, party: Pokemon[]): ModifierType | null {
    const tmGenerators = [
        modifierTypes.ANYTM_COMMON,
        modifierTypes.ANYTM_GREAT,
        modifierTypes.ANYTM_ULTRA,
        modifierTypes.TM_COMMON,
        modifierTypes.TM_GREAT,
        modifierTypes.TM_ULTRA
    ];

    const shuffledGenerators = [...tmGenerators];
    ...
    for (const tmGenerator of shuffledGenerators) {
        const tmModifierType = tmGenerator();
        if (tmModifierType instanceof ModifierTypeGenerator) {
            const generated = tmModifierType.generateType(party);
            if (generated !== null) {
                if (!generated.id) {
                    generated.withIdFromFunc(tmGenerator);
                }
                return generated;
            }
        } else {
...
function getNewModifierTypeOption(party: Pokemon[], poolType: ModifierPoolType, tier?: ModifierTier, upgradeCount?: integer, retryCount: integer = 0, pathNodeFilter?: PathNodeTypeFilter): ModifierTypeOption | null {
    ...
    switch (poolType) {
        case ModifierPoolType.PLAYER:
            ...
            break;
        case ModifierPoolType.COLLECTOR:
            thresholds = collectorModifierPoolThresholds;
            break;
        case ModifierPoolType.WILD:
    ...
    if (tier === undefined) {
        ...
        let divisionFactor = 1.45;
        if (scene.gameData.hasPermaModifierByType(PermaType.PERMA_BETTER_LUCK_3)) {
            divisionFactor = 1;
        } else if (scene.gameData.hasPermaModifierByType(PermaType.PERMA_BETTER_LUCK_2)) {
            divisionFactor = 1.2;
        }
        ...
        if (modifierType instanceof ModifierTypeGenerator) {
            ...
            if (modifierType === null) {
                if (pathNodeFilter === PathNodeTypeFilter.LOW_TIER_MOVE_UPGRADE) {
                    const regularUpgradeModifiers = [new WeightedModifierType(modifierTypes.MOVE_UPGRADE, 1)];
                    const regularModifierType = regularUpgradeModifiers[0].modifierType;
                    if (regularModifierType instanceof ModifierTypeGenerator) {
                        modifierType = (regularModifierType as ModifierTypeGenerator).generateType(party);
                    } else {
                        modifierType = regularModifierType;
                    }
                }
                if (modifierType === null) {
                    return null;
    ...
    if (modifierType instanceof ModifierTypeGenerator) {
        ...
        if (modifierType === null) {
            if (retryCount >= 20) {
                const tmFallback = getRandomTmFallbackForTier(tier, party);
                if (tmFallback) {
                    return new ModifierTypeOption(tmFallback, upgradeCount!);
                }
            }
...
export class QuestModifierType extends ModifierType {
    ...
    get name(): string {
        const data = this.config?.questUnlockData;
        if (data) {
            const rt = data.rewardType as RewardType;
            const speciesName = this.resolveQuestSpeciesDisplayName(data);
            if (speciesName) {
                if (rt === RewardType.GLITCH_FORM_A || rt === RewardType.GLITCH_FORM_B ||
                    rt === RewardType.GLITCH_FORM_C || rt === RewardType.GLITCH_FORM_D ||
                    rt === RewardType.GLITCH_FORM_E) {
                    return i18next.t("modifierSelectUiHandler:questNameGlitch", { name: speciesName, defaultValue: `Glitch ${speciesName} Quest` });
                }
                if (rt === RewardType.SMITTY_FORM || rt === RewardType.SMITTY_FORM_B) {
                    return i18next.t("modifierSelectUiHandler:questNameSmitty", { name: speciesName, defaultValue: `Smitty ${speciesName} Quest` });
                }
                if (rt === RewardType.NEW_MOVES_FOR_SPECIES) {
                    return i18next.t("modifierSelectUiHandler:questNameUltimate", { name: speciesName, defaultValue: `Ultimate ${speciesName} Quest` });
                }
            }
            if (rt === RewardType.GAME_MODE) {
                const modeName = this.resolveQuestModeName(data);
                if (modeName) {
                    return i18next.t("modifierSelectUiHandler:questNameNewMode", { name: modeName, defaultValue: `New ${modeName} Quest` });
                }
            }
        }
    ...
    private resolveQuestSpeciesDisplayName(data: QuestUnlockData): string | null {
        try {
            const rid: any = data.rewardId;
            const ids: any[] = Array.isArray(rid) ? rid : [rid];
            const speciesId = ids.find(v => typeof v === "number");
            if (typeof speciesId !== "number") return null;
            const species = getPokemonSpecies(speciesId);
            return typeof species?.getName === "function" ? species.getName() : null;
        } catch { return null; }
    }

    private resolveQuestModeName(data: QuestUnlockData): string | null {
        try {
            const modeId = data.rewardId as unknown as GameModes;
            if (typeof modeId !== "number") return null;
            return GameMode.getModeName(modeId);
        } catch { return null; }
    }
    ...
    getTooltipDescription(scene: BattleScene): string {
        ...
        const rewardText = this.config?.questUnlockData?.rewardText;
        if (rewardText) {
            sections.push("");
            sections.push(rewardText);
        }
...
export const taurosElectricHitModifier = questModifierTypes.PERMA_HIT_QUEST("TAUROS_ELECTRIC_HIT_QUEST", {
    ...
    task: i18next.t("quests:TAUROS_ELECTRIC_HIT_QUEST.task")
});
export const kecleonColorChangeModifier = questModifierTypes.PERMA_USE_ABILITY_QUEST("KECLEON_COLOR_CHANGE_QUEST", {
    ...
    task: i18next.t("quests:KECLEON_COLOR_CHANGE_QUEST.task")
});
...
export const hitmonchanStatIncreaseModifier = questModifierTypes.PERMA_MOVE_QUEST("HITMONCHAN_STAT_INCREASE_QUEST", {
    ...
    task: i18next.t("quests:HITMONCHAN_STAT_INCREASE_QUEST.task")
});
...
export const ambipomGigaImpactModifier = questModifierTypes.PERMA_MOVE_QUEST("AMBIPOM_GIGA_IMPACT_QUEST", {
    ...
    task: i18next.t("quests:AMBIPOM_GIGA_IMPACT_QUEST.task")
});
...
export const chandelureRestModifier = questModifierTypes.PERMA_MOVE_QUEST("CHANDELURE_REST_QUEST", {
    ...
    task: i18next.t("quests:CHANDELURE_REST_QUEST.task")
});
...
export const eiscueRockKnockoutModifier = questModifierTypes.PERMA_KNOCKOUT_QUEST("EISCUE_ROCK_KNOCKOUT_QUEST", {
    ...
    task: i18next.t("quests:EISCUE_ROCK_KNOCKOUT_QUEST.task")
});
export const zangooseSeviperKnockoutModifier = questModifierTypes.PERMA_KNOCKOUT_QUEST("ZANGOOSE_SEVIPER_KNOCKOUT_QUEST", {
    ...
    task: i18next.t("quests:ZANGOOSE_SEVIPER_KNOCKOUT_QUEST.task")
});
export const seviperZangooseKnockoutModifier = questModifierTypes.PERMA_KNOCKOUT_QUEST("SEVIPER_ZANGOOSE_KNOCKOUT_QUEST", {
    ...
    task: i18next.t("quests:SEVIPER_ZANGOOSE_KNOCKOUT_QUEST.task")
});
export const trubbishPoisonDefeatModifier = questModifierTypes.PERMA_KNOCKOUT_QUEST("TRUBBISH_POISON_DEFEAT_QUEST", {
    ...
    task: i18next.t("quests:TRUBBISH_POISON_DEFEAT_QUEST.task")
});
...
export const ninetalesStoredPowerKnockoutModifier = questModifierTypes.PERMA_KNOCKOUT_QUEST("NINETALES_STORED_POWER_KNOCKOUT_QUEST", {
    ...
    task: i18next.t("quests:NINETALES_STORED_POWER_KNOCKOUT_QUEST.task")
});
...
export const charmanderNightmareWinModifier = questModifierTypes.PERMA_WIN_QUEST("CHARMANDER_NIGHTMARE_WIN_QUEST", {
    ...
    condition: (scene: BattleScene) => {
        ...
        return charmander && charmander.getFormKey() == SpeciesFormKey.GLITCH;
    },
    goalCount: 1,
    questUnlockData: {
        rewardType: RewardType.SMITTY_FORM,
...
let _smittyFormQuestModifiers: Record<string, ReturnType<typeof createSmittyFormQuest>> | null = null;
export const smittyFormQuestModifiers = new Proxy({} as Record<string, ReturnType<typeof createSmittyFormQuest>>, {
  get(target, prop) {
    if (!_smittyFormQuestModifiers) {
      _smittyFormQuestModifiers = Object.entries(SMITTY_CONSOLE_CODES).reduce((acc, [formName, consoleCode]) => {
        ...
        acc[consoleCode] = createSmittyFormQuest({
          ...
          name: i18next.t("modifierSelectUiHandler:questNameSmitty", { name: displayName, defaultValue: `Smitty ${displayName} Quest` }),
          formName,
          isUniversal,
          questId,
          consoleCode
    ...
    return _smittyFormQuestModifiers[prop as string];
  },
  ownKeys() {
    if (!_smittyFormQuestModifiers) {

      const _ = smittyFormQuestModifiers['__force_init__'];
    }
    return _smittyFormQuestModifiers ? Object.keys(_smittyFormQuestModifiers) : [];
  }
});
...
export function getDisabledModifierIds(scene: BattleScene): Set<string> {
    const ids = new Set<string>();
    if (!scene.moveUpgradesEnabledForRun) {
        ids.add("MOVE_UPGRADE");
        ids.add("LOW_TIER_MOVE_UPGRADE");
    }
    if (!scene.statSwitchersEnabledForRun) {
        ids.add("STAT_SWITCHER");
    }
    if (!scene.releaseItemsEnabledForRun) {
        ids.add("STAT_SACRIFICE");
        ids.add("TYPE_SACRIFICE");
        ids.add("MOVE_SACRIFICE");
        ids.add("ABILITY_SACRIFICE");
        ids.add("PASSIVE_ABILITY_SACRIFICE");
        ids.add("SACRIFICE_TOGGLE");
    }
    if (!scene.ivScannerEnabledForRun) {
        ids.add("IV_SCANNER");
    }
    if (!scene.mapEnabledForRun) {
        ids.add("MAP");
    }
    return ids;
}