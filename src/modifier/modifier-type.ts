...
export class ModifierType {
  ...
  get name(): string {
    const text = i18next.t(`${this.localeKey}.name` as any);
    return text == undefined ? this.localeKey : text;
  }

  getDescription(scene: BattleScene): string {
    const text = i18next.t(`${this.localeKey}.description` as any);
    return text == undefined ? this.localeKey : text;
  }
  ...
  constructor(lowTierUpgrade: boolean = false, filterUpgrades?: FilterUpgrades) {
    super((party: Pokemon[], pregenArgs?: any[]) => {
      ...
      if (pregenArgs && pregenArgs.length > 13) {
        ...
        if (maybeFilter) {
          this.filterUpgrades = maybeFilter;
        }
      }
  ...
  private generateRandomMoveUpgrade(party: Pokemon[]): MoveUpgradeModifierType | null {
    ...
    for (const p of availablePokemon) {
      for (const m of p.getMoveset()) {
        ...
        if (isEnemy && scene.enemyMovesUpgradedForWave.moves.includes(m.moveId)) {
          continue;
        }
        pairs.push({ p, m });
      }
...
function createModifierTypeOption(modifierTypeFunc: ModifierTypeFunc, upgradeCount: number, cost: number = 500, scene?: BattleScene): ModifierTypeOption {
  ...
  if (modifierType instanceof QuestModifierTypeGenerator) {
    ...
    if (scene) {
      modifierType.assignScene(scene);
    }
    const GenModifierType = modifierType.generateType([], [0]);
    if (!GenModifierType.id) {
      GenModifierType.withIdFromFunc(modifierTypeFunc);
  ...
  const getPermaModifiersByRarity = (rarity: number): ModifierTypeOption[] => {
    return Object.entries(modifierTypes)
      .filter(([key, factory]) => {
        ...
        if (key === "PERMA_COLLECTED_TYPE") {
          return false;
        }

        if (!key.startsWith("PERMA_")) {
          return false;
        }
...
export class AddPokemonModifierType extends ModifierType {
  ...
  getDescription(scene: BattleScene): string {
    ...
    return this.isDraft
      ? `${baseDescription} ${i18next.t("modifierType:common.morePokeInfo")}`
      : `${baseDescription} ${i18next.t("modifierType:common.morePokeInfo")} ${i18next.t("modifierType:common.glitchPieceCost")}`;
  }
  ...
  constructor(isDraft: boolean = false) {
    super((party: Pokemon[], pregenArgs?: any[]) => {
      ...
      const allRandomPokemon = allSpecies.filter(s => {
        if (s.speciesId === Species.ETERNATUS) {
          return false;
        }
      ...
      if (allRandomPokemon.length === 0) {
        return null;
      }

      const level = scene.currentBattle ? scene.currentBattle.getLvlForWave() : 5;
      const newPokemon = scene.addPlayerPokemon(Utils.randSeedItem(allRandomPokemon), level, 0, 0);
...
export class CollectedTypeModifierType extends PokemonHeldItemModifierType implements GeneratedPersistentModifierType {
  ...
  get name(): string {
    ...
    if (scene?.ui?.getMode) {
      ...
      if (mode === Mode.SHOP_SELECT || mode === Mode.MODIFIER_SELECT || scene.currentPhase?.constructor.name === "ModifierRewardPhase") {
    ...
    super((party: Pokemon[], pregenArgs?: any[]) => {
      ...
      if (!this.onlySmittyItems) {
        formChangeItemPool = [...new Set(party.filter(p =>
          ...
          const formChangeItemTriggers = formChanges.filter(fc => {
          ...
          if (p.species.speciesId === Species.NECROZMA) {
            const necrozmaItems = new Set<FormChangeItem>();
        ...
        const speciesSpecificSmittyItems = party.flatMap(p => {
          ...
          return speciesFormChanges
            .filter(fc => {
              ...
              if (!trigger || trigger?.name === currentForm.formName) {
                return false;
              }
...
export class TrainerBondAbilityModifierType extends ModifierType {
  ...
  getPregenArgs(): any[] {
    return [this.championId, this.ability, this.bonusChance, this.gender];
  }
}
...
export class PokemonAltBuildModifierType extends PokemonModifierType {
  ...
  get name(): string {
    const buildName = i18next.t(`pokemonAltBuild:${this.altBuild.id}.name`);
  ...
  getDescription(scene: BattleScene): string {
    const buildName = i18next.t(`pokemonAltBuild:${this.altBuild.id}.name`);
    const description = i18next.t(`pokemonAltBuild:${this.altBuild.id}.description`);
  ...
  getPregenArgs(): any[] {
    return [this.altBuild.id, this.targetRank];
  }
}
...
export class TeraAbilityModifierType extends ModifierType implements GeneratedPersistentModifierType {
  ...
  get name(): string {
    ...
    const typeName = Utils.toReadableString(Type[this.teraType]);
...
export class ChampionPokemonStatBoosterModifierType extends PokemonModifierType {
  ...
  constructor(championId: string, stats: Stat[], boostPercent: number = 0.01, championTypes?: Type[]) {
  ...
  getPregenArgs(): any[] {
    return [this.championId, this.stats, this.boostPercent, this.championTypes];
  }
}
  ...
  constructor() {
    super((party: Pokemon[], pregenArgs?: any[]) => {
      if (pregenArgs && pregenArgs.length >= 2) {
        ...
        return new ChampionPokemonStatBoosterModifierType(championId, stats, boostPercent ?? 0.01, championTypes);
      }
      const scene = party?.[0]?.scene as BattleScene;
      const championId = (scene as any)?.gameData?.selectedChampionId as string | undefined;
      const champData = (scene as any)?.gameData?.activeSkillTree?.championData as PlayableChampionData | undefined;
...
const modifierPool: ModifierPool = {
  [ModifierTier.COMMON]: [
    new WeightedModifierType(modifierTypes.DNA_SPLICERS, 24),
    ...
    new WeightedModifierType(modifierTypes.SUPER_POTION, (party: Pokemon[]) => {
      if (shouldHideHealingModifier(party)) {
        return 0;
      }
...
export function getPlayerShopModifierTypeOptionsForWave(scene: BattleScene, baseCost: integer): ModifierTypeOption[] | null {
  ...
  const options = [
    [
      ...
      ...(Utils.randSeedInt(100) <= 1 && isNuzlightQuestCompleted ? [createOptionForGenerator(modifierTypes.STAT_SACRIFICE, 0, baseCost *3)] : []),
      ...
      ...(Utils.randSeedInt(100) < 5 && isNuzlightQuestCompleted ? [createOptionForGenerator(modifierTypes.MOVE_UPGRADE, 0, baseCost * 5)] : []),
      ...
      ...(Utils.randSeedInt(100) < 3 ? [new ModifierTypeOption(modifierTypes.GLITCH_PIECE(), 0, 0)] : []),
    ],
    [
      ...(hasShop ? [
...
export function getRandomRewardType(hasSpecies: boolean): {
  ...
  if (hasSpecies) {
    if (rand < 70) {
      ...
      if (moneyTier < 40) {
        rewardId = "PERMA_MONEY_1";
      } else if (moneyTier < 70) {
        rewardId = "PERMA_MONEY_2";
      } else if (moneyTier < 85) {
        rewardId = "PERMA_MONEY_3";
      } else if (moneyTier < 95) {
        rewardId = "PERMA_MONEY_4";
      } else {
        rewardId = "PERMA_MONEY_5";
      }
    ...
    if (rand < 40) {
      ...
      if (moneyTier < 20) {
        rewardId = "PERMA_MONEY_1";
      } else if (moneyTier < 50) {
        rewardId = "PERMA_MONEY_2";
      } else if (moneyTier < 75) {
        rewardId = "PERMA_MONEY_3";
      } else if (moneyTier < 90) {
        rewardId = "PERMA_MONEY_4";
      } else {
        rewardId = "PERMA_MONEY_5";
      }
